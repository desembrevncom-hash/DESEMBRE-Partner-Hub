...
async function main() {
  ...
  report('PERMISSION', 'Anon cannot build', !anonEdgeRes.ok, 
         `Got status: ${anonEdgeRes.status}, Error: ${anonEdgeRes.data?.error || 'unknown'}`);

  // Create a test product
  const testProductId = 88801;
  await sbRest(`product_knowledge?product_id=eq.${testProductId}`, { method: 'DELETE', token: adminToken });
  ...
  report('VALIDATION', 'Draft product cannot be built', !draftBuildRes.ok, 
  ...
  report('BUILD', 'Build embedding for approved product', buildRes.ok,
         buildRes.ok ? `Created ${buildRes.data?.chunkCount} chunks` : JSON.stringify(buildRes.data));

  // Check DB state after build
  const stateRes = await sbRest(`product_knowledge?id=eq.${testPk.id}&select=build_status,knowledge_version,last_embedded_at`, { token: adminToken });
  const state = stateRes.data?.[0];
  report('DB_STATE', 'Build status is completed', state?.build_status === 'completed', `Got ${state?.build_status}`);
  report('DB_STATE', 'Knowledge version is set', !!state?.knowledge_version, `Got v${state?.knowledge_version}`);
  report('DB_STATE', 'Last embedded at is set', !!state?.last_embedded_at);

  // 4. Check chunks
  const chunksRes = await sbRest(`product_knowledge_chunks?product_id=eq.${testProductId}&select=id,is_active,knowledge_version`, { token: adminToken });
  report('CHUNKS', 'Chunks created and active', chunksRes.data?.length > 0 && chunksRes.data[0].is_active === true, 
  ...
  report('REBUILD', 'Rebuild embedding successful', rebuildRes.ok, 
         rebuildRes.ok ? `Created ${rebuildRes.data?.chunkCount} new chunks` : JSON.stringify(rebuildRes.data));

  // Check chunks after rebuild
  const chunksRes2 = await sbRest(`product_knowledge_chunks?product_id=eq.${testProductId}&select=id,is_active,knowledge_version`, { token: adminToken });
  ...
  report('REBUILD_STATE', 'Old chunks are inactive', inactiveChunks.length > 0, `Found ${inactiveChunks.length} inactive chunks`);
  report('REBUILD_STATE', 'New chunks are active', activeChunks.length > 0, `Found ${activeChunks.length} active chunks`);
  
  const newStateRes = await sbRest(`product_knowledge?id=eq.${testPk.id}&select=knowledge_version`, { token: adminToken });
  const newVer = newStateRes.data?.[0]?.knowledge_version;
  report('REBUILD_STATE', 'Knowledge version incremented', newVer > state?.knowledge_version, 
