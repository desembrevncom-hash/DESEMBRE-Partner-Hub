...
async function main() {
  ...
  if (!adminAuth.access_token) {
    console.log(
      "⚠️  Admin login failed:",
      adminAuth.error_description || adminAuth.msg || JSON.stringify(adminAuth),
    );
  ...
  for (const product of csvProducts) {
    ...
    report(
      "IMPORT_CSV",
      `Insert product ${product.product_id}`,
      res.ok,
  ...
  report(
    "IMPORT_JSON",
    `Insert product ${jsonProduct.product_id}`,
    jsonRes.ok,
  ...
  report(
    "IMPORT_TEXT",
    `Insert product ${textProduct.product_id}`,
    textRes.ok,
  ...
  report(
    "DUPLICATE",
    "Duplicate insert blocked (Skip scenario)",
    !dupInsertRes.ok,
  ...
  report(
    "DUPLICATE",
    "Overwrite existing product 99901",
    overwriteRes.ok,
    overwriteRes.ok
      ? `Updated benefits: "${overwriteRes.data?.[0]?.benefits?.substring(0, 40)}..."`
      : JSON.stringify(overwriteRes.data),
  );
  ...
  if (dbRes.ok && Array.isArray(dbRes.data)) {
    ...
    report(
      "DB_STATE",
      `Found ${rows.length} test products`,
      rows.length === 5,
  ...
  if (logsRes.ok && Array.isArray(logsRes.data) && logsRes.data.length > 0) {
    ...
    report(
      "IMPORT_LOG",
      "source_type correct",
      latestLog.source_type === "csv",
      `Expected 'csv', got '${latestLog.source_type}'`,
    );
    report(
      "IMPORT_LOG",
      "success_count = 3",
      latestLog.success_count === 3,
    ...
    report(
      "IMPORT_LOG",
      "metadata.sourceType present",
      !!meta?.sourceType,
      meta?.sourceType || "missing",
    );
    report(
      "IMPORT_LOG",
      "metadata.duplicateAction present",
      !!meta?.duplicateAction,
  ...
  const anonBlocked =
    !anonLogsRes.ok || (Array.isArray(anonLogsRes.data) && anonLogsRes.data.length === 0);
  report(
    "RLS",
    "Anon cannot read import_logs",
    anonBlocked,
    anonLogsRes.ok
      ? `Got ${Array.isArray(anonLogsRes.data) ? anonLogsRes.data.length : 0} rows`
      : `HTTP ${anonLogsRes.status}`,
  );
  ...
  report(
    "RLS",
    "Anon cannot insert product_knowledge",
    !anonWriteRes.ok,
  ...
  if (chunksRes.ok) {
    ...
    report(
      "RAG",
      "No chunks exist for draft test products",
      chunkCount === 0,
  ...
  report(
    "RAG",
    "match_product_chunks filters qa_status=approved",
    hasApprovedFilter,
  ...
  report(
    "RAG",
    "match_product_chunks filters is_active=true",
    hasActiveFilter,
  ...
  report(
    "CODE",
    "Import lib sets qa_status=draft on insert",
    importLib.includes("qa_status: 'draft'"),
    "",
  );
  report(
    "CODE",
    "Import lib resets approved_by=null on insert",
    importLib.includes("approved_by: null"),
    "",
  );
  report(
    "CODE",
    "Import lib resets approved_at=null on insert",
  ...
  report(
    "CODE",
    "Route checks isAdminOrSubAdmin",
  ...
  report(
    "CODE",
    "ManagerWorkspace has /admin/product-import link",
