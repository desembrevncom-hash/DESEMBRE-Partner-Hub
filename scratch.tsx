...
import { Check, ChevronsUpDown, Map, CheckSquare } from "lucide-react";
...
return <div/>;
  ...
  const handleBulkDispatch = async () => {
    ...
    if (dispatchAction !== 'revoke' && dispatchStaffId === 'none' && dispatchAction !== 'change_stage') {
      toast.error("Vui lòng chọn nhân viên đích!");
      return;
    }
    ...
    try {
      if (dispatchAction === 'change_stage') {
        ...
        if (error) throw error;
        toast.success(`Đã đổi stage thành công cho ${selectedCustomers.length} khách!`);
        ...
        if (error) throw error;
        toast.success(`Đã thu hồi thành công ${selectedCustomers.length} khách!`);
      } else {
        const isSaleAssignment = dispatchAction === 'assign_sale';
        const { error } = await supabase.rpc('bulk_assign_customers', {
          p_customer_ids: selectedCustomers,
          p_sale_id: isSaleAssignment ? dispatchStaffId : null,
          p_update_sale: isSaleAssignment,
          p_tele_id: !isSaleAssignment ? dispatchStaffId : null,
          p_update_tele: !isSaleAssignment,
          p_reason: dispatchReason
        });
        if (error) throw error;
        toast.success(`Đã phân công thành công ${selectedCustomers.length} khách!`);
      }
      ...
      toast.error("Lỗi điều phối: " + error.message);
    } finally {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      if (cIds.length > 0) {
        ...
        const { data: mngData, error: mngError } = await supabase.rpc('get_customer_management_summary', {
           p_customer_ids: cIds
        });
        if (!mngError && mngData) {
           const mngMap = new Map(mngData.map((i: any) => [i.customer_id, i] as [string, any]));
           processed.forEach((c: any) => {
              const mng = mngMap.get(c.id);
              if (mng) {
                 c.task_summary = mng;
              }
           });
        } else if (mngError) {
           console.error("fetch management summary error:", mngError);
        }
      }
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter(c => {
      ...
      const { getRiskFlags } = require('@/lib/customerHealth');
      const flags = getRiskFlags(c);
      
      let matchSmart = true;
      if (smartFilter === "unassigned") matchSmart = flags.includes("Unassigned");
      if (smartFilter === "overdue") matchSmart = flags.includes("Overdue Follow-up");
      if (smartFilter === "no_interaction") matchSmart = flags.includes("No Follow-up") || flags.includes("Inactive 7d");
      if (smartFilter === "pending_revoke") matchSmart = flags.includes("Pending Revoke");
      if (smartFilter === "at_risk") matchSmart = flags.includes("At Risk");
      ...
      if (smartFilter === "no_social") matchSmart = flags.includes("Missing Social");
  ...
  const adminStats = useMemo(() => {
    ...
    customers.forEach(c => {
       ...
       const flags = getRiskFlags(c);
       if (flags.includes('Unassigned')) dispatchStats.unassigned++;
       if (flags.includes('Overdue Follow-up')) dispatchStats.overdue++;
       if (flags.includes('Missing Social')) dispatchStats.missingSocial++;
       if (flags.includes('No Follow-up') || flags.includes('Inactive 7d')) dispatchStats.noInteraction++;
       if (flags.includes('Pending Revoke')) dispatchStats.pendingRevoke++;
       if (flags.includes('At Risk')) dispatchStats.atRisk++;
    });
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20 font-sans antialiased">
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl space-y-8">
        ...
        {isManager && adminStats && (
          <div className="bg-white p-4 rounded-[24px] border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center text-xs font-bold text-slate-600">
             <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md flex items-center gap-1.5"><Layers className="w-3.5 h-3.5" /> Lead Dispatch</span>
             
             <button onClick={() => setSmartFilter(smartFilter === "unassigned" ? "all" : "unassigned")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "unassigned" ? 'bg-indigo-600 border-indigo-600 text-white' : 'hover:bg-slate-50 border-slate-200'}`}>
                <UserPlus className="w-3.5 h-3.5" /> Chưa phân bổ ({adminStats.dispatchStats.unassigned})
             </button>
             
             <button onClick={() => setSmartFilter(smartFilter === "overdue" ? "all" : "overdue")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "overdue" ? 'bg-amber-500 border-amber-500 text-white' : 'hover:bg-amber-50 border-amber-200 text-amber-700'}`}>
                <Clock className="w-3.5 h-3.5" /> Quá hạn ({adminStats.dispatchStats.overdue})
             </button>
             
             <button onClick={() => setSmartFilter(smartFilter === "no_social" ? "all" : "no_social")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "no_social" ? 'bg-rose-500 border-rose-500 text-white' : 'hover:bg-rose-50 border-rose-200 text-rose-700'}`}>
                <AlertCircle className="w-3.5 h-3.5" /> Thiếu Social ({adminStats.dispatchStats.missingSocial})
             </button>
             
             <button onClick={() => setSmartFilter(smartFilter === "no_interaction" ? "all" : "no_interaction")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "no_interaction" ? 'bg-slate-600 border-slate-600 text-white' : 'hover:bg-slate-50 border-slate-200 text-slate-700'}`}>
                <Activity className="w-3.5 h-3.5" /> Không có tương tác ({adminStats.dispatchStats.noInteraction})
             </button>

             <button onClick={() => setSmartFilter(smartFilter === "at_risk" ? "all" : "at_risk")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "at_risk" ? 'bg-red-600 border-red-600 text-white' : 'hover:bg-red-50 border-red-200 text-red-700'}`}>
                <XCircle className="w-3.5 h-3.5" /> Nguy cơ rời bỏ ({adminStats.dispatchStats.atRisk})
             </button>
             
             <button onClick={() => setSmartFilter(smartFilter === "pending_revoke" ? "all" : "pending_revoke")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${smartFilter === "pending_revoke" ? 'bg-purple-600 border-purple-600 text-white' : 'hover:bg-purple-50 border-purple-200 text-purple-700'}`}>
                <MoreVertical className="w-3.5 h-3.5" /> Chờ thu hồi ({adminStats.dispatchStats.pendingRevoke})
             </button>
          </div>
        )}
      ...
      {selectedCustomers.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-white p-2 rounded-2xl shadow-2xl border border-slate-200 flex items-center gap-2 animate-in slide-in-from-bottom-5">
           ...
           <Button size="sm" variant="outline" className="h-8 text-xs font-bold text-slate-700" onClick={() => { setDispatchAction('change_stage'); setIsDispatchDialogOpen(true); }}>
              <ArrowRight className="w-3.5 h-3.5 mr-1" /> Chuyển Stage
           </Button>
      ...
      <Dialog open={isDispatchDialogOpen} onOpenChange={setIsDispatchDialogOpen}>
        <DialogContent className="sm:max-w-md rounded-[24px]">
          ...
          <div className="space-y-4 py-4">
             ...
             <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 uppercase">
                  Lý do {dispatchAction === 'revoke' ? 'thu hồi' : 'điều phối'} <span className="text-red-500">*</span>
                </label>
                <textarea 
