...
async function run() {
  ...
  const { data: batch, error: batchErr } = await supabase.from('customer_import_batches').insert({
  ...
  if (batchErr) { console.error('Insert batch error:', batchErr); return; }
  ...
  if (rowErr) { console.error('Insert row error:', rowErr); return; }
  ...
  if (!nonAuthErr && nonAuthData.user) {
      ...
      const { data: batch2, error: batchErr2 } = await supabase.from('customer_import_batches').insert({
      ...
      if (batch2) {
          const { data: rpcData2, error: rpcError2 } = await supabase.rpc('confirm_customer_import_batch', {
