...
import fs from 'fs';
import { mapImportRow, validateImportRow, detectDuplicateInFile, buildImportSummary } from './src/lib/customers/importValidation';
...
async function runTest() {
  ...
  if (phonesToCheck.length > 0) {
    const { data: pData } = await supabase.from('customers').select('id, normalized_phone').in('normalized_phone', phonesToCheck);
    if (pData) existingCustomers = [...existingCustomers, ...pData];
  }
  if (emailsToCheck.length > 0) {
    const { data: eData } = await supabase.from('customers').select('id, normalized_email').in('normalized_email', emailsToCheck);
  ...
  const { data: batchQuery } = await supabase
    .from('customer_import_batches')
    .select('id, file_name, total_rows, valid_rows, invalid_rows, duplicate_rows, status, import_mode, created_at')
  ...
  const { data: rowsQuery } = await supabase
    ...
    .select('row_number, name, phone, email, validation_status, import_action, matched_customer_id, duplicate_reason')
    .eq('batch_id', batchId)
    .order('row_number');
