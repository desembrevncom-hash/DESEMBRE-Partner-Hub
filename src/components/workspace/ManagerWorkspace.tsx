  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [customersRes, tasksRes, notifsRes, companyRes, reclaimRes] = await Promise.all([
        ...
        supabase
          .from("customer_tasks")
          .select("*, customer:customers(name, facility_name, phone)")
        ...
        supabase
          .from("customers")
          .select("id")
          .in("ownership_status", ["at_risk", "reclaimable"])
          .is("deleted_at", null),
      ]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            <>
              <Link to="/admin/crm-ops">
                <Button
                  size="sm"
                  variant="outline"
                  className="border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 hover:text-indigo-800 text-[10px] font-black uppercase h-9 px-3 rounded-xl flex items-center gap-1.5 shadow-sm"
                >
          ...
          <Link to="/admin/hub">
            <Button
              size="sm"
              className="bg-slate-900 text-white hover:bg-black text-[11px] font-black uppercase h-9 px-5 rounded-xl flex items-center gap-2 shadow-lg shadow-slate-200 ml-2"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black text-red-900 uppercase tracking-wide">
                Yêu cầu thu hồi cần xử lý
              </h4>
              <p className="text-[11px] font-bold text-slate-600 mt-0.5">
                Có <span className="text-red-600 font-extrabold">{data.reclaimCount}</span> khách
                hàng đang trong trạng thái cảnh báo hoặc quá hạn chăm sóc.
              </p>
            </div>
          </div>
          <Link to="/admin/reclamation">
            <Button
              size="sm"
              className="bg-red-600 hover:bg-red-700 text-white rounded-xl text-[10px] font-black uppercase tracking-wider h-9 px-4"
            >
              Xử lý ngay
            </Button>
          </Link>
