...
export const WorkspaceCalendarCard: React.FC<WorkspaceCalendarCardProps> = ({
  events,
  onRefresh,
}) => {
  ...
  const handleDeleteTask = async () => {
    ...
    if (!window.confirm("Bạn có chắc chắn muốn xóa lịch hẹn/công việc này khỏi hệ thống không?"))
      return;
    ...
    <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
      ...
      <div className="flex-1 p-2 grid grid-cols-7 gap-px bg-slate-100">
        ...
        {["T2", "T3", "T4", "T5", "T6", "T7", "CN"].map((day) => (
          <div
            key={day}
            className="bg-white py-2 text-center text-[10px] font-black text-slate-400 uppercase"
          >
            {day}
          </div>
        ))}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[11px] font-bold ${isToday ? "bg-primary text-white w-5 h-5 rounded-full flex items-center justify-center -mr-1 shadow-sm shadow-primary/30" : "text-slate-400"}`}
                >
                  {format(day, "d")}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-0.5 overflow-hidden mt-1">
                {dayEvents.slice(0, 3).map((ev, i) => {
                  ...
                  if (ev.status === "completed") {
                    bgClass =
                      "bg-emerald-50 text-emerald-600 border-emerald-100 line-through opacity-70";
                  } else {
        ...
        <DialogContent className="sm:max-w-[425px] rounded-3xl">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label className="text-[11px] font-bold uppercase text-slate-400 text-center">
                  Thời gian
                </Label>
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-slate-800 mt-0.5 group-hover/cal-cust:text-indigo-600 transition-colors">
                            {selectedEvent?.customer?.name ||
                              selectedEvent?.customer?.facility_name ||
                ...
                <div>
                  ...
                  <div className="mt-1">
                    {selectedEvent?.status === "completed" ? (
                      <span className="text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-md border border-emerald-200 font-bold">
                        ✓ Xong
                      </span>
