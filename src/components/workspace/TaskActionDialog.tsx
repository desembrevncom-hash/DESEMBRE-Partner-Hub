...
export const TaskActionDialog: React.FC<TaskActionDialogProps> = ({
  ...
  const [nextActionValue, setNextActionValue] = useState<'follow_up' | 'transfer_to_sale'>('follow_up');
    ...
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md p-6">
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0">
          ...
          <div>
            <DialogTitle className="text-sm font-black uppercase tracking-wider text-slate-900">{title}</DialogTitle>
            <p className="text-[10px] text-slate-450 font-bold uppercase truncate max-w-[300px]">Công việc: {task.title}</p>
          </div>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
          ...
          {action === "start" && (
            <p className="text-xs text-slate-600 font-medium">
              Bạn có chắc chắn muốn chuyển trạng thái công việc này sang <span className="font-bold text-blue-600">Đang xử lý</span>?
            </p>
          )}
          ...
          {action === "completed" && (
            <p className="text-xs text-slate-600 font-medium">
              Bạn xác nhận đã <span className="font-bold text-emerald-600">Hoàn thành</span> công việc này? Hệ thống sẽ ghi nhận lịch sử hoạt động tương ứng.
            </p>
          )}
          ...
          {action === "no_answer" && (
            <div className="space-y-3">
              <p className="text-xs text-slate-600 font-medium">
                Hệ thống sẽ cập nhật trạng thái kết quả thành <span className="font-bold text-red-600">Không nghe máy</span>. Bạn có muốn đặt thêm lịch hẹn gọi lại không?
              </p>
              <div className="space-y-1.5 pt-2">
                <Label className="text-[10px] font-black uppercase text-slate-500">Lịch gọi lại (Tùy chọn)</Label>
                <Input 
                  type="datetime-local" 
                  value={dateTimeValue}
          ...
          {action === "wrong_number" && (
            <div className="space-y-2">
              <p className="text-xs text-slate-600 font-medium">
                Xác nhận số điện thoại của khách hàng bị <span className="font-bold text-red-650">Sai số / Liên lạc lỗi</span>? 
              </p>
              <div className="bg-amber-50 border border-amber-100 p-3 rounded-lg text-[10px] text-amber-800 font-bold flex items-start gap-2">
                ...
                <span>Lưu ý: Hệ thống sẽ tự động chuyển trạng thái của khách hàng liên kết sang Lost.</span>
              </div>
            </div>
          )}
          ...
          {action === "interested" && (
            <div className="space-y-3">
              ...
              <div className="space-y-1.5 pt-2">
                <Label className="text-[10px] font-black uppercase text-slate-500">Bước xử lý tiếp theo</Label>
                <Select 
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="transfer_to_sale">Đạt chất lượng - Bàn giao cho Sale chăm sóc</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          {action === "call_back_later" && (
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black uppercase text-slate-500">Thời gian gọi lại <span className="text-red-500">*</span></Label>
                <Input 
                  type="datetime-local" 
                  value={dateTimeValue}
                  onChange={(e) => setDateTimeValue(e.target.value)}
          ...
          {action === "transfer_to_sale" && (
            <p className="text-xs text-slate-600 font-medium">
              Xác nhận gửi yêu cầu bàn giao khách hàng này cho <span className="font-bold text-indigo-600">Sale phụ trách</span>? Hệ thống sẽ cập nhật trạng thái khách hàng sang Qualified.
            </p>
          )}
        ...
        <div className="flex items-center justify-end gap-2.5">
          <Button variant="ghost" size="sm" onClick={onClose} disabled={loading} className="text-xs font-bold">
            Hủy
          </Button>
          <Button 
            size="sm" 
            ...
            className={`text-xs font-bold px-4 ${
              action === "wrong_number" || action === "no_answer" ? "bg-red-600 hover:bg-red-500" :
              action === "completed" ? "bg-emerald-600 hover:bg-emerald-500" :
