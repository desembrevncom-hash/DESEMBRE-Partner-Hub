  ...
  return (
    <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-xl shadow-slate-200/50 flex flex-col gap-3 min-w-[280px]">
      <div className="flex justify-between items-start gap-4">
        ...
        <Badge
          variant="outline"
          className="text-[10px] px-1.5 py-0 uppercase bg-slate-50 font-bold border-slate-200"
        >
        ...
        <div className="bg-slate-50 rounded p-1.5 flex flex-col gap-0.5">
          ...
          <span className="text-slate-700 font-medium flex items-center gap-1">
            ...
            {state.lastInteractionTime
              ? format(new Date(state.lastInteractionTime), "dd/MM/yyyy")
              : "Chưa có"}
          </span>
        </div>
        <div
          className={`rounded p-1.5 flex flex-col gap-0.5 ${state.urgency === "overdue" ? "bg-rose-50" : "bg-slate-50"}`}
        >
          <span
            className={`text-[10px] font-bold uppercase ${state.urgency === "overdue" ? "text-rose-500" : "text-slate-400"}`}
          >
            Hẹn tiếp theo
          </span>
          <span
            className={`font-medium flex items-center gap-1 ${state.urgency === "overdue" ? "text-rose-700" : "text-slate-700"}`}
          >
            <Calendar
              className={`w-3 h-3 ${state.urgency === "overdue" ? "text-rose-500" : "text-slate-400"}`}
            />
        ...
        <div className="flex flex-wrap gap-1 mt-1">
          {signals.slice(0, 2).map((sig: any, idx: number) => (
            <Badge
              key={idx}
              variant="outline"
              className="text-[9px] px-1 py-0 bg-amber-50 text-amber-700 border-amber-200 flex items-center gap-1"
            >
          ...
          {signals.length > 2 && (
            <Badge variant="outline" className="text-[9px] px-1 py-0 bg-slate-50 text-slate-500">
              +{signals.length - 2}
            </Badge>
          )}
        </div>
