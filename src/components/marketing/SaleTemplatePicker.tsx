...
import { Search, Copy, Check, Filter, Layers, Clock, ShieldAlert, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export const MOCK_TEMPLATES: MarketingTemplate[] = [
  { id: '1', name: 'Bảng giá điều trị Nám 2026', category: 'Báo giá', channel: 'Zalo', content: 'Kính gửi Quý Spa, Desembre xin gửi bảng giá phác đồ điều trị Nám mới nhất...' },
  { id: '2', name: 'Phác đồ Siêu vi tảo', category: 'Phác đồ', channel: 'Zalo', content: 'Hướng dẫn chi tiết các bước trong phác đồ Siêu vi tảo Desembre...' },
  { id: '3', name: 'Mời Workshop Hà Nội T6', category: 'Workshop', channel: 'Zalo', content: 'Trân trọng mời anh/chị tham gia sự kiện Workshop Đào tạo kỹ thuật tại Hà Nội...' },
  { id: '4', name: 'Hỏi thăm định kỳ', category: 'CSKH', channel: 'Zalo', content: 'Chào chị, dạo này Spa mình lượng khách ổn định không ạ? Em gửi chị thông tin...' },
  { id: '5', name: 'Giới thiệu sp mới (Upsell)', category: 'Upsell', channel: 'Zalo', content: 'Sắp tới bên em ra mắt dòng tinh chất phục hồi cao cấp, chị có muốn nhận sample dùng thử không ạ?' },
  { id: '6', name: 'Nhắc lịch lấy hàng', category: 'Chăm sóc lại', channel: 'Zalo', content: 'Chị ơi, sản phẩm X đợt trước chị lấy sắp hết rồi đúng không ạ? Em lên đơn mới cho chị nhé!' }
];
...
export function SaleTemplatePicker({ customerId, customer, onCopy }: SaleTemplatePickerProps) {
  ...
  const [recentIds, setRecentIds] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('recent_marketing_templates') || '[]'); }
    catch { return []; }
  });
  ...
  const recentTemplates = useMemo(() => {
    return recentIds.map(id => MOCK_TEMPLATES.find(t => t.id === id)).filter(Boolean) as MarketingTemplate[];
  }, [recentIds]);
  ...
  const senderStatus = useMemo(() => {
    ...
    const zaloSender = personalSenders.find(s => s.platform?.toLowerCase().includes('zalo') && s.is_active);
          ...
          {CATEGORIES.map(cat => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[10px] font-bold whitespace-nowrap transition-colors ${
                category === cat ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
              {cat}
            </button>
      ...
      <ScrollArea className="flex-1 p-4 bg-slate-50/50">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {recentTemplates.map(t => (
                <TemplateCard key={t.id} template={t} onCopy={() => handleCopy(t)} isCopied={copiedId === t.id} isLoading={copying === t.id} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          {filteredTemplates.length === 0 ? (
            <div className="text-center py-6 text-slate-400 text-xs">Không tìm thấy mẫu phù hợp.</div>
          ) : (
...
function TemplateCard({ template, onCopy, isCopied, isLoading }: {
  ...
  return (
    <div className="p-3 bg-white rounded-xl border border-slate-200 hover:border-purple-300 hover:shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-2">
        <div>
          <Badge variant="secondary" className="text-[9px] bg-slate-100 text-slate-600 mb-1">{template.category}</Badge>
