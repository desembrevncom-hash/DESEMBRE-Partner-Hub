...
const DEFAULT_TEMPLATES: Template[] = [
  {
    ...
    type: "zalo",
    content: "Chào {{customer_name}} từ {{facility_name}}, em là {{sale_name}} từ Desembre Việt Nam. Rất vui được kết nối với anh/chị để hỗ trợ về các dòng dược mỹ phẩm cao cấp cho Spa mình. Em xin phép gửi Profile công ty để anh/chị tham khảo trước nhé!"
  },
  {
    id: "quote-followup",
    ...
    type: "zalo",
    content: "Dạ anh/chị {{customer_name}} ơi, không biết anh/chị đã xem qua bản báo giá các bộ sản phẩm Nám/Mụn em gửi hôm qua chưa ạ? Nếu có chỗ nào cần điều chỉnh chiết khấu hoặc số lượng cho {{facility_name}}, anh/chị cứ bảo em nhé!"
  },
  {
    id: "post-purchase",
    ...
    type: "zalo",
    content: "Chào anh/chị {{customer_name}}, bộ sản phẩm Spa mình nhập tuần trước dùng khách phản hồi thế nào rồi ạ? Em gửi thêm quy trình kỹ thuật chuẩn của hãng để kỹ thuật viên nhà mình tham khảo thêm nhé. Cần hỗ trợ gì cứ nhắn em ạ!"
  },
  {
    id: "promotion",
    ...
    type: "zalo",
    content: "Tin vui cho {{facility_name}} đây ạ! Desembre đang có chương trình tri ân dành riêng cho các chủ Spa thân thiết. Khi nhập đơn hàng trong tháng này, anh/chị sẽ được tặng thêm bộ kit Demo trị giá 2.5tr. Số lượng có hạn nên em nhắn anh/chị ưu tiên trước ạ!"
  }
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 h-[600px]">
          ...
          <div className="md:col-span-2 bg-slate-50/50 border-r border-slate-100 flex flex-col">
            <div className="p-6 border-b border-slate-100 bg-white">
              <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-4">Thư viện Mẫu</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
              {filteredTemplates.map(t => (
                ...
                >
                  <p className="text-[9px] font-black text-indigo-500 uppercase tracking-wider mb-1">{t.category}</p>
            ...
            <div className="flex-1 p-8">
              {selectedId ? (
                <div className="space-y-6">
                   <div className="bg-slate-50 rounded-[32px] p-8 border border-slate-100 relative group">
                      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                         <Badge className="bg-white text-slate-400 border-slate-100 text-[8px] font-black">PREVIEW MODE</Badge>
                      </div>
                   ...
                   <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest px-2">
                      ...
                      <Badge variant="outline" className="text-[9px] border-slate-200">Tên khách</Badge>
                      <Badge variant="outline" className="text-[9px] border-slate-200">Tên Spa</Badge>
                      <Badge variant="outline" className="text-[9px] border-slate-200">Tên Sale</Badge>
                   </div>
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-4">
                   ...
                   <p className="text-[10px] font-black uppercase tracking-[0.2em]">Chọn một mẫu để bắt đầu</p>
                </div>
              )}
            </div>
