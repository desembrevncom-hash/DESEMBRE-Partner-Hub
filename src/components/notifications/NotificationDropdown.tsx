...
export const NotificationDropdown: React.FC<NotificationDropdownProps> = ({
  ...
  const getNotificationIcon = (type: NotificationType) => {
    ...
    switch (type) {
      case 'lead_assigned':
        return <div className="p-2 bg-emerald-50 rounded-xl"><UserPlus className={`${iconClass} text-emerald-500`} /></div>;
      case 'event_upcoming':
        return <div className="p-2 bg-indigo-50 rounded-xl"><CalendarDays className={`${iconClass} text-indigo-500`} /></div>;
      case 'task_overdue':
      case 'followup_overdue':
        return <div className="p-2 bg-red-50 rounded-xl"><AlertTriangle className={`${iconClass} text-red-500`} /></div>;
      case 'channel_approval_required':
      case 'duplicate_risk':
        return <div className="p-2 bg-amber-50 rounded-xl"><ShieldAlert className={`${iconClass} text-amber-500`} /></div>;
      case 'order_attention':
        return <div className="p-2 bg-blue-50 rounded-xl"><Package className={`${iconClass} text-blue-500`} /></div>;
      case 'system':
      default:
        return <div className="p-2 bg-slate-50 rounded-xl"><Bell className={`${iconClass} text-slate-400`} /></div>;
    }
  };
      ...
      <div className="max-h-[400px] overflow-y-auto no-scrollbar relative">
        {notifications.length > 0 ? (
              ...
              <div className="flex-1 min-w-0 space-y-1">
                 <div className="flex justify-between items-start gap-2">
                    <div className="flex flex-col gap-1 min-w-0">
                      <span className={`text-xs leading-tight font-bold truncate ${n.status === 'unread' ? "text-slate-950 font-black" : "text-slate-600"}`}>
                        {n.title}
                      </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {n.status === 'unread' && <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />}
                    </div>
                 </div>
                 {n.message && (
          ...
          <div className="py-16 flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Tất cả đều ổn thỏa</p>
          </div>
        )}
      </div>
    </DropdownMenuContent>
