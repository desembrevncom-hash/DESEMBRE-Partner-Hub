...
export const AISuggestionCard: React.FC<AISuggestionCardProps> = ({
  ...
  const handleFeedback = async (type: "thumbs_up" | "thumbs_down") => {
    ...
    try {
      ...
      toast.success(
        type === "thumbs_up" ? "👍 Cảm ơn! Phản hồi đã ghi nhận." : "👎 Đã ghi nhận. Sẽ cải thiện.",
      );
    } catch (err: any) {
          ...
          <div key={s.id} className={`p-3 rounded-xl border ${getColorClass(s.type)} space-y-2`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 font-bold text-xs">
                ...
                {s.priority === "high" && (
                  <span className="ml-2 px-1.5 py-0.5 bg-rose-100 text-rose-700 rounded text-[9px] uppercase font-black">
                    Ưu tiên
                  </span>
                )}
              </div>
