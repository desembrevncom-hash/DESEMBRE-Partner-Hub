...
export function RoutingReviewDialog({ open, onOpenChange, user }: RoutingReviewDialogProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: custData, error: custErr } = await supabase
        .from("customers")
        .select(
          "id, name, facility_name, phone, latitude, longitude, customer_distance_type, customer_channel, care_model",
        )
        .is("deleted_at", null)
  ...
  const mismatchedCount = reviewedData.filter(
    (d: any) => d.status === "Lệch phân tuyến" || d.status === "Thiếu dữ liệu",
  ).length;
  ...
  const handleApplyAllMismatched = async () => {
    const toUpdate = reviewedData.filter(
      (d: any) => d.status === "Lệch phân tuyến" || d.status === "Thiếu dữ liệu",
    );
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn áp dụng đồng loạt gợi ý cho ${toUpdate.length} khách hàng bị lệch?`,
      )
    )
      return;
              ...
              <div className="flex items-center justify-between bg-blue-50 p-3 rounded-xl border border-blue-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-blue-900">
                    Tìm thấy <strong className="text-blue-700">{mismatchedCount}</strong> khách hàng
                    có phân tuyến chưa chuẩn so với gợi ý.
                  </span>
                </div>
                          ...
                          >
                            ...
                            <td className="px-3 py-2">
                              {item.status === "Thiếu dữ liệu" ? (
                                ...
                                <div className="space-y-0.5 text-[9px] font-bold text-slate-600">
                                  ...
                                  <div>
                                    Kênh: {getCustomerChannelLabel(item.currentRouting.channel)}
                                  </div>
                                  <div>
                                    Mô hình: {getCareModelLabel(item.currentRouting.careModel)}
                                  </div>
                                </div>
                              )}
                            </td>
                            <td className="px-3 py-2">
                              <div className="space-y-0.5 text-[9px] font-bold text-emerald-700 bg-emerald-50 p-1.5 rounded-md border border-emerald-100">
                                ...
                                <div>
                                  Mô hình: {getCareModelLabel(item.suggestedRouting.careModel)}
                                </div>
                              </div>
                            </td>
                            <td className="px-3 py-2 text-center">
                              {item.status === "Đúng" ? (
                                <Badge className="bg-emerald-100 text-emerald-700 border-none hover:bg-emerald-100 cursor-default shadow-none">
                                  Đúng
                                </Badge>
                              ) : item.status === "Lệch phân tuyến" ? (
                                <Badge className="bg-amber-100 text-amber-700 border-none hover:bg-amber-100 cursor-default shadow-none animate-pulse">
                                  Lệch
                                </Badge>
                              ) : (
                                <Badge className="bg-rose-100 text-rose-700 border-none hover:bg-rose-100 cursor-default shadow-none">
                                  Thiếu
                                </Badge>
                              )}
                            </td>
