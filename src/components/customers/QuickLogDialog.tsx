...
export function QuickLogDialog({ customer, isOpen, onClose, onSuccess }: QuickLogDialogProps) {
  ...
  const [type, setType] = useState<"call" | "zalo_message" | "online_consultation" | "note">(
    "call",
  );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[450px] rounded-[32px] border-none shadow-2xl p-8">
        ...
        <div className="py-6 space-y-6">
          <div
            className={`grid gap-3 ${isManager ? "grid-cols-1 max-w-[150px] mx-auto" : "grid-cols-3"}`}
          >
            {types.map((t) => (
              <button
                key={t.id}
        ...
        <DialogFooter className="gap-3">
          <Button
            variant="ghost"
            onClick={onClose}
            className="h-11 md:h-10 rounded-xl font-bold text-slate-400"
          >
            Hủy
          </Button>
          <Button
