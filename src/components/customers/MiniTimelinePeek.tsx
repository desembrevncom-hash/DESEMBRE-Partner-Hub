...
export function MiniTimelinePeek({ customer, children }: MiniTimelinePeekProps) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'call': return <Phone className="w-3 h-3 text-emerald-500" />;
      case 'zalo': return <MessageSquare className="w-3 h-3 text-blue-500" />;
      case 'note': return <StickyNote className="w-3 h-3 text-amber-500" />;
  ...
  return (
    <Popover onOpenChange={loadActivities}>
      ...
      <PopoverContent className="w-80 p-3" align="end">
        <h4 className="text-xs font-bold text-slate-800 mb-3 uppercase tracking-wider">Lịch sử gần đây</h4>
        {loading ? (
          ...
          <div className="space-y-3">
            {activities.map(act => (
              <div key={act.id} className="flex gap-2 items-start text-xs">
                ...
                <div>
                  <div className="font-medium text-slate-700">{act.content || act.activity_type}</div>
                  <div className="text-[10px] text-slate-400">{format(new Date(act.created_at), 'dd/MM/yyyy HH:mm')}</div>
                </div>
              </div>
            ))}
          </div>
