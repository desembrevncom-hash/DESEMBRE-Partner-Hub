  ...
  const suggestions = useMemo(() => {
    ...
    items.forEach(it => {
      ...
      if (isNaN(pId) && it.product_name) {
        const p = PRODUCTS.find(p => p.name.includes(it.product_name) || it.product_name.includes(p.name));
        if (p) pId = p.id;
      }
    ...
    const customerConcerns: string[] = Array.isArray(customer?.skin_concerns) 
      ...
      : (customer?.skin_concern_focus ? [customer.skin_concern_focus] : []);
  ...
  return (
    <div className="p-5 bg-gradient-to-br from-indigo-500/5 to-purple-500/5 rounded-2xl border border-indigo-100 shadow-3xs space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px] font-black uppercase text-indigo-700 tracking-wider">AI / Rule-based Gợi ý Upsell</span>
      </div>
      
      <div className="space-y-3">
        {suggestions.map((s, i) => (
          <div key={s.product_id} className="bg-white p-3.5 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden group">
            {s.isRestock && (
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {s.reasons.map((r: string, idx: number) => (
                <Badge key={idx} variant="outline" className="text-[9px] bg-slate-50 border-slate-200 text-slate-600 px-1.5 font-bold uppercase">
                  {r}
                </Badge>
              ))}
            </div>
