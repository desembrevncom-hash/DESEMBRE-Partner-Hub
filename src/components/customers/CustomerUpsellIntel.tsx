...
export const CustomerUpsellIntel: React.FC<CustomerUpsellIntelProps> = ({
  orders,
  items,
  totalSpend,
}) => {
  ...
  const analysis = useMemo(() => {
    ...
    const purchasedLines = {
      ...
      serum: items.some((it) =>
        /(serum|tế bào gốc|ampoule|concentrate)/i.test(it.product_name || ""),
      ),
    ...
    if (purchasedLines.serum && !purchasedLines.cream) {
      crossSells.push(
        "Gợi ý bán kèm **Kem dưỡng khóa ẩm** để tối ưu hóa hiệu quả khóa dưỡng chất của Tinh chất Ampoule đã mua.",
      );
    }
    if (purchasedLines.cleansing && !purchasedLines.sunscreen) {
      crossSells.push(
        "Tư vấn thêm **Kem chống nắng Desembre** để bảo vệ nền da sau khi làm sạch sâu.",
      );
    }
    if (!purchasedLines.cleansing && purchasedLines.cream) {
      crossSells.push(
        "Chào bán thêm dòng **Tẩy trang & Sữa rửa mặt** để tối ưu hóa nền da trước dưỡng.",
      );
    }
    if (purchasedLines.sunscreen && !purchasedLines.cleansing) {
      crossSells.push(
        "Tư vấn **Tẩy trang chuyên dụng** để làm sạch sâu lớp kem chống nắng vật lý vào cuối ngày.",
      );
    }
    if (crossSells.length === 0) {
      crossSells.push(
        "Khuyên dùng **Tế bào gốc Desembre Activator** - dòng sản phẩm điều trị Spa bán chạy nhất.",
      );
    }
    ...
    return (
      <div className="p-8 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
        ...
        <p className="text-xs text-slate-500 mt-2 font-medium max-w-sm mx-auto leading-relaxed">
          Chưa đủ dữ liệu để gợi ý upsell. Hãy chốt đơn hàng đầu tiên của khách hàng để kích hoạt
          trí tuệ gợi ý bán hàng!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 text-left">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            ...
            <span className="text-xs font-bold text-slate-400">
              Tiến tới {tierProgress.nextTier}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="divide-y divide-slate-50">
            {topProducts.map((p, idx) => (
              <div key={idx} className="py-2.5 flex items-center justify-between text-xs">
                ...
                <span className="font-black text-slate-400 shrink-0 bg-slate-50 px-2 py-0.5 rounded-md">
                  SL: {p.qty}
                </span>
              </div>
            ))}
        ...
        <div className="p-5 bg-white rounded-2xl border border-slate-200/60 shadow-2xs space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px] font-black uppercase">
            <div
              className={`p-2 rounded-xl border flex items-center justify-between ${purchasedLines.cleansing ? "bg-emerald-50/50 border-emerald-100 text-emerald-700" : "bg-slate-50 border-slate-100 text-slate-400"}`}
            >
              <span>Làm sạch</span>
              <span>{purchasedLines.cleansing ? "✓" : "○"}</span>
            </div>
            <div
              className={`p-2 rounded-xl border flex items-center justify-between ${purchasedLines.serum ? "bg-emerald-50/50 border-emerald-100 text-emerald-700" : "bg-slate-50 border-slate-100 text-slate-400"}`}
            >
              <span>Ampoule</span>
              <span>{purchasedLines.serum ? "✓" : "○"}</span>
            </div>
            <div
              className={`p-2 rounded-xl border flex items-center justify-between ${purchasedLines.sunscreen ? "bg-emerald-50/50 border-emerald-100 text-emerald-700" : "bg-slate-50 border-slate-100 text-slate-400"}`}
            >
              <span>Chống nắng</span>
              <span>{purchasedLines.sunscreen ? "✓" : "○"}</span>
            </div>
            <div
              className={`p-2 rounded-xl border flex items-center justify-between ${purchasedLines.cream ? "bg-emerald-50/50 border-emerald-100 text-emerald-700" : "bg-slate-50 border-slate-100 text-slate-400"}`}
            >
              <span>Kem dưỡng</span>
          ...
          <div className="space-y-2">
            {forecastItems.length > 0 ? (
              forecastItems.map((f, idx) => (
                <div
                  key={idx}
                  className={`p-3 rounded-xl border flex flex-col gap-1 ${f.severity === "danger" ? "bg-rose-50 border-rose-100 text-rose-700" : "bg-amber-50 border-amber-100 text-amber-800"}`}
                >
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-wider py-4 text-center">
                Tất cả sản phẩm đều trong hạn dùng
              </p>
            )}
          </div>
        </div>
        ...
        <div className="p-5 bg-white rounded-2xl border border-slate-200/60 shadow-2xs space-y-3">
          ...
          <div className="space-y-2.5">
            {crossSells.map((c, idx) => (
              <div
                key={idx}
                className="p-3 bg-indigo-50/40 rounded-xl border border-indigo-100 text-xs font-medium text-slate-700 leading-relaxed"
              >
                {c.split("**").map((part, index) =>
                  index % 2 === 1 ? (
                    <strong key={index} className="text-indigo-700 font-extrabold">
                      {part}
                    </strong>
                  ) : (
                    part
      ...
      {unpurchasedLines.length > 0 && (
        <div className="p-5 bg-white rounded-2xl border border-slate-200/60 shadow-2xs space-y-3">
          <span className="text-[11px] font-black uppercase text-slate-400 tracking-wider">
            Dòng sản phẩm chưa từng mua
          </span>
          <div className="flex flex-wrap gap-2">
            {unpurchasedLines.map((l, idx) => (
              <span
                key={idx}
                className="text-[10px] font-bold px-3 py-1 rounded-full bg-slate-50 border border-slate-200 text-slate-500"
              >
                {l}
              </span>
            ))}
