    ...
    return (
      <Card className="bg-slate-50 dark:bg-slate-900 border-dashed border-2 shadow-sm mb-4">
        <CardContent className="pt-6 text-center">
          ...
          <h3 className="font-semibold text-slate-700 dark:text-slate-300 mb-1">AI Trợ lý bán hàng</h3>
          <p className="text-sm text-slate-500 mb-4">Nhận gợi ý hành động tiếp theo dựa trên lịch sử tương tác</p>
          <Button onClick={generateSuggestion} size="sm" className="bg-blue-600 hover:bg-blue-700 text-white">
          ...
          <>
            ...
            {suggestion.risk_flags && suggestion.risk_flags.length > 0 && (
              <div className="flex flex-col gap-2">
                {suggestion.risk_flags.map((flag, idx) => (
                  <div key={idx} className={`flex items-start p-2 rounded-md text-sm ${flag.severity === 'high' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400'}`}>
            ...
            {suggestion.next_best_action && (
              <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-blue-100 dark:border-slate-700 shadow-sm">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold uppercase text-slate-500 tracking-wider">Hành động ưu tiên</span>
                  {suggestion.next_best_action.priority === 'high' && <Badge variant="destructive" className="h-5 text-[10px]">Ưu tiên cao</Badge>}
                </div>
                <div className="font-medium text-slate-800 dark:text-slate-200 mb-1">
                  ...
                  {!['call','zalo','facebook','schedule'].includes(suggestion.next_best_action.action) && `💡 ${suggestion.next_best_action.action}`}
                </div>
                <p className="text-sm text-slate-600 dark:text-slate-400">{suggestion.next_best_action.reason}</p>
                ...
                {suggestion.recommended_channel && (
                  <div className="mt-2 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-700 p-1.5 rounded inline-block">
                    Đề xuất kênh: <span className="font-semibold uppercase">{suggestion.recommended_channel.platform}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {suggestion.message_suggestion && suggestion.message_suggestion.text && (
              <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-all">
                ...
                >
                  <div>
                    <span className="text-xs font-semibold uppercase text-slate-500 tracking-wider">Gợi ý tin nhắn ({suggestion.message_suggestion.platform})</span>
                    {!showMessage && (
                ...
                {showMessage && (
                  <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex items-center justify-end mt-3 gap-2">
                      <Button variant="outline" size="sm" onClick={copyMessage} className="h-8 text-xs">
                        <Copy className="h-3 w-3 mr-1.5" /> Copy
                      </Button>
                      <Button size="sm" className="h-8 text-xs bg-blue-600" onClick={() => handleUpdateStatus('accepted')}>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <Button variant="ghost" size="sm" className="h-6 text-xs text-slate-500 hover:text-red-600" onClick={() => handleUpdateStatus('dismissed')}>
                Bỏ qua
              </Button>
            </div>
          </>
