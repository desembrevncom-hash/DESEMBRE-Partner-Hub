...
export const AdminCustomerInsights: React.FC<AdminCustomerInsightsProps> = ({
  ...
  const inactiveDays = Math.floor(
    (new Date().getTime() - new Date(customer.last_contacted_at || customer.created_at).getTime()) /
      (1000 * 3600 * 24),
  );
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 flex flex-col justify-center items-center text-center">
            ...
            <div
              className={`text-xs font-black ${inactiveDays > 7 ? "text-rose-600" : "text-emerald-600"}`}
            >
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-3 gap-2">
            ...
            <Button onClick={onAssignTele} size="sm" variant="outline" className="text-xs px-2 h-8">
              ...
              <span className="truncate">Đổi Tele</span>
            </Button>
            <Button
              onClick={onRevoke}
              size="sm"
              variant="outline"
              className="text-xs px-2 h-8 text-rose-600 hover:text-rose-700 hover:bg-rose-50 border-rose-200"
            >
              <UserMinus className="w-3 h-3 mr-1 shrink-0" />{" "}
              <span className="truncate">Thu hồi</span>
            </Button>
          </div>
          <Button
            onClick={onAdminNote}
            size="sm"
            className="w-full text-xs h-8 bg-slate-800 text-white hover:bg-slate-900"
          >
