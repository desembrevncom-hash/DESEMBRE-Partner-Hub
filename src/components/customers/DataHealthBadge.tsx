...
export function DataHealthBadge({
  ...
  const badgeContent = (
    <Badge
      variant="outline"
      className={`flex items-center gap-1 cursor-help transition-colors ${result.badgeClassName} ${className}`}
    >
    ...
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="inline-flex shrink-0">{badgeContent}</div>
        </TooltipTrigger>
        <TooltipContent className="max-w-xs p-3 space-y-2 rounded-xl shadow-xl border-slate-100 bg-white">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
            <Icon
              className={`w-4 h-4 ${result.severity === "danger" ? "text-rose-500" : result.severity === "warning" ? "text-amber-500" : "text-emerald-500"}`}
            />
          ...
          {result.severity === "ok" ? (
            <p className="text-xs text-slate-500 font-medium">
              Dữ liệu khách hàng đầy đủ và đang được chăm sóc tốt.
            </p>
          ) : (
            <ul className="space-y-1">
              {result.reasons.map((r, i) => (
                <li
                  key={i}
                  className="text-[11px] font-medium text-slate-600 flex items-start gap-1.5"
                >
