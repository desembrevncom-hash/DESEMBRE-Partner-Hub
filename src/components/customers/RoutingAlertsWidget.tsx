...
export function RoutingAlertsWidget() {
    ...
    const fetchAlertsAndStaff = async () => {
      ...
      try {
        const [
          ...
          supabase.from("customers").select("id, name, facility_name, customer_distance_type").is("deleted_at", null).is("owner_sale_id", null).in("customer_distance_type", ["near_company", "same_city"]).limit(50),
          supabase.from("customers").select("id, name, facility_name, customer_distance_type").is("deleted_at", null).is("owner_tele_id", null).in("customer_distance_type", ["province", "far_city"]).limit(50),
          supabase.from("customers").select("id, name, facility_name").is("deleted_at", null).or("latitude.is.null,longitude.is.null").limit(50),
  ...
  const toggleSelectTele = (id: string) => {
    setSelectedTeleIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
  };
  ...
  const handleAssignConfirm = async () => {
    ...
    for (const target of assignTargets) {
      try {
        ...
        const { error: updErr } = await supabase.from("customers").update(updates).eq("id", target.id);
    ...
    if (failedCount === 0) {
      ...
      toast.warning(`Hoàn tất với lỗi. Thành công: ${successCount}. Thất bại: ${failedCount}. (Khách lỗi: ${failedNames.join(', ')})`, { duration: 8000 });
    }
        ...
        <DialogContent className="sm:max-w-[400px] rounded-3xl p-6 border-slate-100 shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs font-bold text-slate-500 uppercase mt-1 tracking-widest">
              Bạn đang gán {assignTargets.length} khách cho {assignType === 'sale' ? 'Sale' : 'Trưởng Tele'}.
              Thao tác này sẽ tạo thông báo, nhắc lịch task và activity cho từng khách.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="pt-2">
            ...
            <Button onClick={handleAssignConfirm} disabled={saving || !selectedStaffId} className="rounded-xl text-xs font-black uppercase bg-slate-900 hover:bg-black px-6">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : `Xác nhận gán ${assignTargets.length} khách`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
