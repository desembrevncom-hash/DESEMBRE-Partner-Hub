...
export function SafeCustomerImportDialog({
  open,
  onOpenChange,
  onReviewBatch,
}: SafeCustomerImportDialogProps) {
  ...
  const [filter, setFilter] = useState<"all" | "valid" | "invalid" | "duplicate" | "warning">(
    "all",
  );
        ...
        rows = rows.map((r) => {
          ...
          if (r.normalized_phone) {
            const match = existingCustomers.find(
              (ec) => ec.normalized_phone === r.normalized_phone,
            );
            if (match) matchedId = match.id;
          }
          if (!matchedId && r.normalized_email) {
            const match = existingCustomers.find(
              (ec) => ec.normalized_email === r.normalized_email,
            );
            if (match) matchedId = match.id;
          }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col overflow-hidden bg-slate-50">
        <DialogHeader className="bg-white p-6 border-b border-slate-200 -mx-6 -mt-6">
          ...
          <DialogDescription>
            Luồng import an toàn 3 bước. Hệ thống sẽ phân tích, kiểm tra trùng lặp và đưa vào
            Staging.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="p-8">
              <div className="max-w-lg mx-auto bg-white border border-dashed border-slate-300 rounded-2xl p-10 text-center shadow-sm">
                ...
                <p className="text-sm text-slate-500 mb-6">
                  Hỗ trợ file CSV, XLSX (Tối đa 2.000 dòng).
                  <br />
                        ...
                        <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                          ...
                          <td className="px-4 py-2">
                            ...
                            <div className="text-slate-500">
                              {r.business_name || r.facility_name}
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-2">
                            {r.error_message && (
                              <div className="text-rose-600 flex items-center gap-1">
                                <XCircle className="w-3 h-3" /> {r.error_message}
                              </div>
                            )}
                            {r.warning_message && (
                              <div className="text-blue-600 flex items-center gap-1">
                                <AlertTriangle className="w-3 h-3" /> {r.warning_message}
                              </div>
                            )}
                          </td>
                        </tr>
            ...
            <div className="p-8">
              <div className="max-w-lg mx-auto bg-white border border-emerald-200 rounded-2xl p-10 text-center shadow-sm">
                ...
                <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl text-[11px] text-amber-800 text-left mb-6">
                  ...
                  <strong>Lưu ý:</strong> Dữ liệu đang ở vùng đệm chờ duyệt. Vui lòng chuyển sang
                  bước Review để kiểm tra lại và Confirm Import.
                </div>
          ...
          <DialogFooter className="bg-white p-4 border-t border-slate-200 -mx-6 -mb-6 mt-0">
            <div className="w-full flex justify-between items-center">
              ...
              <div className="flex gap-2 items-center">
                {isProcessing && (
                  <div className="text-[11px] font-medium text-slate-500 mr-2 flex items-center">
                    <Loader2 className="w-3 h-3 animate-spin mr-1" /> Đang lưu Staging {progress}%
                  </div>
                )}
                <Button
