...
import { AlertCircle, CheckCircle2, XCircle, Info, Loader2, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
export function BatchReviewDialog({ batchId, onOpenChange, onConfirmSuccess }: BatchReviewDialogProps) {
  ...
  const [filter, setFilter] = useState<'all' | 'valid' | 'invalid' | 'duplicate' | 'skip' | 'imported'>('all');
  ...
  const filteredRows = rows.filter((row) => {
    if (filter !== 'all') {
      ...
      if (filter === 'valid' && row.validation_status !== 'valid' && row.validation_status !== 'warning') return false;
      if (filter === 'invalid' && row.validation_status !== 'error' && row.validation_status !== 'invalid') return false;
      ...
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
                    ...
                    {paginatedRows.map((row) => (
                      <tr key={row.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3">
                          ...
                          {row.import_action === 'imported' && <span className="text-indigo-600 font-medium text-xs">Imported</span>}
                        ...
                        <td className="px-4 py-3 text-xs text-rose-600">
                          ...
                          {row.duplicate_reason && <span className="text-amber-600 block mt-1">{row.duplicate_reason}</span>}
                        </td>
                      </tr>
                    ))}
              ...
              {totalPages > 1 && (
                <div className="p-3 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-sm shrink-0">
                  <div className="text-slate-500">
                    Hiển thị <span className="font-bold text-slate-700">{paginatedRows.length}</span> trên tổng số <span className="font-bold text-slate-700">{filteredRows.length}</span> dòng
                  </div>
        ...
        <DialogFooter className="pt-4 border-t border-slate-100 flex items-center justify-between">
          <div className="flex-1 flex flex-col justify-center">
            {batch?.status !== 'completed' && batch?.status !== 'processing' && eligibleRows.length > 0 && isAdminOrSubAdmin && (
              <p className="text-xs text-amber-600 flex items-center gap-1.5 font-medium">
                <AlertCircle className="w-4 h-4 shrink-0" />
                Hệ thống chỉ tạo khách hàng mới từ dòng hợp lệ. Không cập nhật khách cũ. Dòng lỗi/trùng sẽ bị bỏ qua.
              </p>
            )}
          </div>
