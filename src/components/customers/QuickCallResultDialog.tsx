...
import { PhoneOff, Heart, Clock, FileText, AlertCircle, PhoneMissed, Loader2, Calendar } from "lucide-react";
...
type ResultType = 'no_answer' | 'interested' | 'call_back' | 'sent_quote' | 'wrong_number' | 'unreachable';
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Lưu nhanh kết quả vừa gọi điện cho khách hàng.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          {(resultType === 'call_back' || resultType === 'no_answer' || resultType === 'interested') && (
            <div className="space-y-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50">
              <Label className="flex items-center gap-1.5 text-xs text-indigo-600"><Calendar className="w-3.5 h-3.5" /> Hẹn gọi lại vào lúc nào?</Label>
              ...
              {selectedPreset === 'tu_chon' && (
                <div className="grid grid-cols-2 gap-4 mt-3 p-3 bg-white rounded-xl border border-slate-200 shadow-sm">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Ngày hẹn</Label>
                    <Input 
                      type="date" 
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Giờ hẹn</Label>
                    <Input 
                      type="time" 
