...
export function InlineQuickNote({ customer, open, onOpenChange, onSaved }: InlineQuickNoteProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] p-5 md:p-6 w-[calc(100%-32px)] rounded-3xl">
        <DialogHeader>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="h-11 md:h-10">
            Hủy
          </Button>
          <Button onClick={handleSave} disabled={loading || !note.trim()} className="h-11 md:h-10">
