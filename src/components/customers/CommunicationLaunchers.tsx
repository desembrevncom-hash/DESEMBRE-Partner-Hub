...
export const CommunicationLaunchers: React.FC<Props> = ({
  ...
  const getCustomerIdentifierObj = (
    platform: CommunicationPlatform,
  ): { value: string; id?: string } => {
    const primaryChannel = customerChannels.find(
      (c) => c.channel_type === platform && c.is_primary,
    );
    if (primaryChannel?.channel_value)
      return { value: primaryChannel.channel_value, id: (primaryChannel as any).id };
    const anyChannel = customerChannels.find((c) => c.channel_type === platform);
    if (anyChannel?.channel_value)
      return { value: anyChannel.channel_value, id: (anyChannel as any).id };
    if ((platform === "zalo" || platform === "phone") && customerPhone)
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-500 mt-1">
              <div>
                ...
                <span className="font-medium text-slate-700">
                  {interactionSummary.last_interaction_at
                    ? new Date(interactionSummary.last_interaction_at).toLocaleDateString("vi-VN")
                    : "-"}
                </span>
              </div>
              <div>
                ...
                <span className="font-medium text-slate-700 capitalize">
                  {interactionSummary.most_used_platform || "-"}
                </span>
              </div>
            </div>
              ...
              <button
                ...
                ${
                  disabled
                    ...
                    : isWarning
                      ? "bg-amber-100 border-amber-200 hover:bg-amber-200 text-amber-900 shadow-sm"
                      : "bg-white border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 shadow-sm"
                }
              `}
              >
                <p.icon
                  className={`w-4 h-4 ${disabled ? p.color : isWarning ? "text-amber-600" : p.activeColor}`}
                />
            ...
            {hasMsgTpl ? (
              <div>
                ...
                >
                  ...
                  {templates
                    .filter((t) => t.channel === selectedPlatform || t.channel === "all")
                    .map((t) => (
