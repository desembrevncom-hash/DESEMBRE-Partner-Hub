...
export function ProductKnowledgeBook() {
  ...
  const filteredList = useMemo(() => {
    return knowledgeList.filter((k) => {
      ...
      const matchesSearch =
        pName.includes(sQuery) || (k.benefits || "").toLowerCase().includes(sQuery);
        ...
        {filteredList.length === 0 ? (
          <div className="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200">
            ...
            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest">
              Chưa có cẩm nang sản phẩm phù hợp
            </p>
          </div>
        ) : (
          filteredList.map((k) => {
              ...
              >
                ...
                >
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-[11px] text-slate-500 line-clamp-2">{k.benefits}</p>
                  </div>
                    ...
                    <div className="pt-3 space-y-3">
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              copyToClipboard(k.sales_pitch, "Sales Pitch");
                            }}
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              copyToClipboard(k.usage_instructions, "Hướng dẫn");
                            }}
                            className="h-6 px-2 text-[9px] text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50"
                          >
                            <Copy className="w-3 h-3 mr-1" /> Copy
                        ...
                        <p className="text-[11px] text-slate-600">{k.usage_instructions}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                        <span className="text-[10px] text-slate-500 font-medium">
                          Chu kỳ mua lại:{" "}
                          <strong className="text-slate-700">{k.restock_cycle_days} ngày</strong>
                        </span>
