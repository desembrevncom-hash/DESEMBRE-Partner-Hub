...
import { renderTemplate, getTemplateSampleData, validateTemplateVariables } from "@/lib/documentTemplates";
...
export const DocumentTemplatePreview: React.FC<DocumentTemplatePreviewProps> = ({ htmlTemplate, templateType }) => {
    ...
    <div className="flex flex-col w-full h-full">
      {!validation.valid && (
        <div className="bg-amber-50 text-amber-800 p-3 text-sm border-b border-amber-200">
          <span className="font-bold">Cảnh báo:</span> Template đang thiếu các biến bắt buộc: {validation.missing.map(m => `{{${m}}}`).join(", ")}
        </div>
      )}
