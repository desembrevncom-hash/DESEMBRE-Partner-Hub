...
const severityConfig: Record<InterventionSeverity, { bg: string, border: string, icon: any, iconColor: string }> = {
  critical: { bg: 'bg-rose-50/50', border: 'border-rose-200/50', icon: AlertOctagon, iconColor: 'text-rose-500' },
  warning: { bg: 'bg-amber-50/50', border: 'border-amber-200/50', icon: AlertTriangle, iconColor: 'text-amber-500' },
  monitor: { bg: 'bg-slate-50/50', border: 'border-slate-200/50', icon: AlertCircle, iconColor: 'text-slate-400' },
};

export function OperationalSuggestionCard({ intervention, onAction }: OperationalSuggestionCardProps) {
  const config = severityConfig[intervention.severity];
  const Icon = config.icon;

  return (
    <div className={`p-4 rounded-xl border ${config.bg} ${config.border} flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all hover:shadow-sm`}>
