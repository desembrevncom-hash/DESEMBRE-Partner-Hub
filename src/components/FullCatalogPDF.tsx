...
export const FullCatalogPDF = ({
  ...
  return (
    <Document
      title={`Desembre Vietnam - Bảng giá sản phẩm ${vatOn ? "(Có VAT)" : "(Chưa VAT)"} ${isSaleView ? "(Giá Sale)" : ""}`}
    >
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.logo}>DESEMBRE VIETNAM</Text>
          <Text style={{ fontSize: 6 }}>
            BẢNG GIÁ NIÊM YẾT {vatOn ? `(ĐÃ CÓ VAT ${Math.round(vatRate * 100)}%)` : "(CHƯA VAT)"}{" "}
            {isSaleView ? "- CHẾ ĐỘ GIÁ SALE (60%)" : ""}
          </Text>
        </View>
              ...
              {items.map((p, pIdx: number) => {
                ...
                return (
                  <View key={p.id} style={styles.card}>
                    {p.imageUrl && <Image src={p.imageUrl} style={styles.productImage} />}
                    ...
                    {salon && salon.price > 0 && (
                      <View style={styles.priceRow}>
                        <Text style={styles.priceLabel}>
                          Professional ({salon.size}){vatOn ? " (+VAT)" : ""}:
                        </Text>
        ...
        <Text style={styles.footer}>
          © {new Date().getFullYear()} Desembre Vietnam. Tất cả giá đã bao gồm các chiết khấu tiêu
          chuẩn.
        </Text>
        <Text
