    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.table}>
        ...
        <View style={[styles.tableRow, styles.tableHeader]}>
          <View style={[styles.tableCol, styles.colNo]}><Text style={styles.cellText}>#</Text></View>
          <View style={[styles.tableCol, styles.colImage]}><Text style={styles.cellText}>Ảnh</Text></View>
          <View style={[styles.tableCol, styles.colName]}><Text style={styles.cellTextLeft}>Sản phẩm / Description</Text></View>
          <View style={[styles.tableCol, styles.colSize]}><Text style={styles.cellText}>Size</Text></View>
          <View style={[styles.tableCol, styles.colPrice]}><Text style={styles.cellTextRight}>Đơn giá</Text></View>
          <View style={[styles.tableCol, styles.colTotal]}><Text style={styles.cellTextRight}>Thành tiền</Text></View>
        </View>

        {/* Rows */}
        {items.map((it: any, idx: number) => (
          <View key={idx} style={styles.tableRow}>
            <View style={[styles.tableCol, styles.colNo]}><Text style={styles.cellText}>{idx + 1}</Text></View>
            <View style={[styles.tableCol, styles.colImage]}>
              {it.image_url ? (
                <Image src={it.image_url} style={{ width: 40, height: 40, objectFit: 'cover', alignSelf: 'center', borderRadius: 4 }} />
              ) : (
                <Text style={{ fontSize: 6, color: '#ccc', textAlign: 'center', marginTop: 15 }}>NO IMG</Text>
              )}
            </View>
            <View style={[styles.tableCol, styles.colName]}>
              <Text style={[styles.cellTextLeft, { fontWeight: 'bold' }]}>{it.product_name}</Text>
              <Text style={{ fontSize: 6, color: '#666', marginTop: 1 }}>{it.size_type === 'retail' ? 'Dòng bán lẻ' : 'Dòng chuyên nghiệp'}</Text>
            </View>
            <View style={[styles.tableCol, styles.colSize]}><Text style={styles.cellText}>{it.size}</Text></View>
            <View style={[styles.tableCol, styles.colPrice]}><Text style={styles.cellTextRight}>{fmt(it.unit_price)}</Text></View>
            <View style={[styles.tableCol, styles.colTotal]}><Text style={styles.cellTextRight}>{fmt(it.unit_price * it.quantity)}</Text></View>
          </View>
        ))}
      </View>
      ...
      <View style={{ marginTop: 40, flexDirection: 'row', justifyContent: 'space-between' }}>
        <View style={{ width: '40%', textAlign: 'center' }}>
          ...
          {quoterPhone && <Text style={{ fontSize: 7, marginTop: 2, color: '#666' }}>{quoterPhone}</Text>}
          {quoterEmail && <Text style={{ fontSize: 7, marginTop: 2, color: '#666' }}>{quoterEmail}</Text>}
          {quoterPhone && (
            <Image 
          ...
          {quoterPhone && <Text style={{ fontSize: 6, marginTop: 2, color: '#999' }}>Quét mã để nhắn Zalo</Text>}
        </View>
        <View style={{ width: '40%', textAlign: 'center' }}>
