...
export function snapshotOrderAttribution(orderTotal: number, customer: any): OrderAttributionSnapshot {
  return {
...
export function calculateCampaignRevenue(orders: any[], campaigns: any[]) {
  ...
  orders.forEach(o => {
    ...
    const campId = o.attributed_campaign_id || (o.customer ? (o.customer.latest_campaign_id || o.customer.first_campaign_id) : null);
...
export function calculateSourceRevenue(orders: any[]) {
  const result: Record<string, { revenue: number; orders: number; qualified: number; leads: number }> = {};

  orders.forEach(o => {
    const source = o.attributed_source || (o.customer ? o.customer.lead_source : 'Unknown') || 'Unknown';
    if (!result[source]) {
