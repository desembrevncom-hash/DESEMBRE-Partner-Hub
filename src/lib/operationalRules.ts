...
export type StaleSignal = 'lead_dead' | 'forgotten' | 'stage_stuck' | 'quote_ignored' | 'no_touchpoint';
...
export function getStaleSignals(customer: any): { signal: StaleSignal, message: string }[] {
  ...
  const lastActivityDate = state.lastInteractionTime ? new Date(state.lastInteractionTime) : new Date(customer.created_at);
  ...
  if ((stage.includes('quote') || stage.includes('proposal')) && daysSinceActivity > 3) {
    signals.push({ signal: 'quote_ignored', message: `Gửi báo giá ${daysSinceActivity} ngày chưa liên hệ lại` });
  }
