...
export const handleNoAnswerTaskAction = async (
  task: any,
  userId?: string,
  ...
  const { error } = await supabase.from("customer_tasks").update(updates).eq("id", task.id);
  if (error) throw error;

  if (task.customer_id) {
    const timeText = nextCallDate
      ? ` - Hẹn gọi lại vào lúc ${new Date(nextCallDate).toLocaleString("vi-VN")}`
      : "";
    await supabase.from("customer_activities").insert([
      {
        customer_id: task.customer_id,
...
export const handleWrongNumberTaskAction = async (task: any, userId?: string) => {
  ...
  if (task.customer_id) {
    ...
    await supabase.from("customers").update({ lifecycle_stage: "lost" }).eq("id", task.customer_id);
  }
};
...
export const handleInterestedTaskAction = async (
  ...
  if (task.customer_id) {
    const actionText =
      nextAction === "transfer_to_sale" ? "Yêu cầu chuyển giao Sale" : "Hẹn follow-up";
    await supabase.from("customer_activities").insert([
      {
        customer_id: task.customer_id,
        created_by: userId,
    ...
    await supabase.from("customers").update(updates).eq("id", task.customer_id);
  }
};

export const handleRescheduleTaskAction = async (
  task: any,
