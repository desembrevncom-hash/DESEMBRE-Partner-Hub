...
export interface OperationalIntervention {
  id: string;
  type: 'OVERLOADED_STAFF' | 'STAGE_BOTTLENECK' | 'SILENT_VIP' | 'STALE_QUOTE' | 'DEAD_LEADS_ACCUMULATION';
...
export function getInterventions(customers: any[], staffMap: Record<string, any>): OperationalIntervention[] {
  ...
  customers.forEach(c => {
    ...
    if (isVIP && !isClosed) {
      const daysSinceActivity = state.lastInteractionTime ? differenceInDays(now, new Date(state.lastInteractionTime)) : differenceInDays(now, new Date(c.created_at));
    ...
    // Check Stale Quote
    const isQuotePending = stage.includes('quote') || stage.includes('proposal') || stage.includes('negotiation');
