                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                        {t.title}
                        {t.is_shared && (
                          <span className="bg-blue-100 text-blue-700 text-[10px] px-1.5 py-0.5 rounded uppercase">
                            Dùng chung
                          </span>
                        )}
                        {t.platform === "all" && (
                          <span className="bg-slate-100 text-slate-600 text-[10px] px-1.5 py-0.5 rounded uppercase">
                            Mọi nền tảng
                          </span>
                        )}
                        {t.platform !== "all" && (
                          <span className="bg-emerald-100 text-emerald-700 text-[10px] px-1.5 py-0.5 rounded uppercase">
                            {t.platform}
                          </span>
                        )}
                      </h3>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            {isEditing && (
              ...
              >
                <X className="w-3 h-3 mr-1" /> Huỷ sửa
              </button>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">
                Nội dung (hỗ trợ biến)
              </label>
              <p className="text-[10px] text-slate-400 mb-2">
                Biến: {"{{customer_name}}, {{spa_name}}, {{sale_name}}, {{phone}}"}
              </p>
              <textarea
                value={form.content}
            ...
            <button
              onClick={handleSave}
              className="w-full bg-primary text-white h-10 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-primary/90"
            >
