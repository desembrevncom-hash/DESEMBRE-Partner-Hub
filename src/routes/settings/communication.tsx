      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm mb-8">
        ...
        <form onSubmit={handleAddAccount} className="flex flex-col gap-4">
          ...
          {newPlatform === "email" && (
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-2">
              ...
              <p className="text-[11px] text-emerald-700 mb-3 leading-relaxed">
                Để hệ thống có thể tự động gửi Email Marketing từ tài khoản của bạn, vui lòng cung
                cấp Mật khẩu ứng dụng (App Password). Mật khẩu này được mã hóa an toàn và hệ thống
                không lưu mật khẩu thật của bạn.
              </p>
              <div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="bg-primary text-white h-[42px] px-6 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-primary/90 transition-colors"
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                      {acc.account_name}
                      {acc.is_default && (
                        <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded uppercase tracking-wide">
                          Mặc định
                        </span>
                      )}
                    </h3>
                    <p className="text-xs text-slate-500 mt-0.5">
                      Kênh: <span className="font-medium capitalize">{acc.platform}</span> · ID:{" "}
                      <span className="font-medium">{acc.account_identifier}</span>
                    </p>
                    {acc.platform === "email" && (
