...
function MarketingDashboardPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "ready" | "no_consent" | "excluded" | "invalid_contact"
  >("all");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const processed = (customers || []).map((c) => {
        ...
        const emailReadiness = getReadinessStatus(
          c,
          "email",
          isDuplicate,
          consentMap,
        ...
        const zaloReadiness = getReadinessStatus(
          c,
          "zalo",
          isDuplicate,
          consentMap,
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-8 text-slate-500 font-medium animate-pulse">
        Đang tải dữ liệu Marketing Readiness...
      </div>
    );
  }
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl space-y-6">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3 items-start">
          ...
          <div>
            <h3 className="font-bold text-amber-800 text-sm">
              Dashboard này chỉ đo lường mức độ sẵn sàng (Readiness)
            </h3>
            <p className="text-amber-700 text-xs mt-1">
              Hệ thống tuân thủ chặt chẽ các chính sách bảo vệ quyền riêng tư & chống Spam hiện
              hành. Chỉ những khách hàng đã Opt-in và có thông tin hợp lệ mới được đưa vào nhóm
              "Ready". Lưu ý: Hiện tại hệ thống chưa kích hoạt tính năng gửi Email/Zalo thực tế.
            </p>
          </div>
        </div>
        ...
        {limitWarning && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex gap-3 items-center text-xs text-blue-700">
            ...
            <span>
              Dữ liệu preview hiện đang bị giới hạn ở mức 5.000 khách hàng mới nhất để đảm bảo hiệu
              suất MVP.
            </span>
          </div>
        )}
            ...
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-emerald-900 mt-1">
                  {currentSummary.ready.toLocaleString()}
                </h3>
              </div>
            ...
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-amber-900 mt-1">
                  {currentSummary.noConsent.toLocaleString()}
                </h3>
              </div>
            ...
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-rose-900 mt-1">
                  {currentSummary.excluded.toLocaleString()}
                </h3>
              </div>
            ...
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-slate-800 mt-1">
                  {currentSummary.invalid.toLocaleString()}
                </h3>
              </div>
        ...
        <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="text-base font-black text-slate-800 flex items-center gap-2">
              <Users className="w-5 h-5 text-indigo-500" /> Preview Audience (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-50 text-xs font-medium text-slate-700">
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((c) => {
                    const readiness =
                      channelFilter === "email" ? c.emailReadiness : c.zaloReadiness;
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <Badge
                            variant="outline"
                            className="bg-slate-100 text-slate-600 border-none font-bold uppercase text-[9px]"
                          >
                            {channelFilter}
                          </Badge>
                        </td>
                        <td className="px-5 py-3 text-center">
                          ...
                          {readiness === "invalid_contact" && (
                            <Badge className="bg-slate-200 text-slate-600 hover:bg-slate-200 border-none font-bold">
                              INVALID
                            </Badge>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-3 text-[11px] text-slate-500">
                          {c.last_contacted_at
                            ? format(new Date(c.last_contacted_at), "dd/MM/yyyy")
                            : "Never"}
                        </td>
                      </tr>
                    );
