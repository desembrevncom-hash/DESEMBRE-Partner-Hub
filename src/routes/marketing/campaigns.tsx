...
function MarketingCampaignsPage() {
  ...
  const canManageCampaign = (camp: Campaign | null) =>
    camp ? isAdminOrSubAdmin || (isSale && camp.created_by === user?.id) : false;
  ...
  const baselineCampaigns: Campaign[] = [
    {
      ...
      message_templates: {
        name: "Mẫu thư mời chuẩn Hội thảo Nám",
        channel: "email_campaign",
        purpose: "marketing_campaign",
      },
      sender_accounts: {
        id: "snd-1",
        name: "Email Marketing Tổng",
        sender_email: "marketing@desembrevn.com",
      },
    ...
    },
    {
      ...
      message_templates: {
        name: "Mẫu thông báo Chính sách Đại lý",
        channel: "email_campaign",
        purpose: "monthly_campaign",
      },
      sender_accounts: {
        id: "snd-2",
        name: "Email Chăm sóc Đại lý",
        sender_email: "partners@desembrevn.com",
      },
    ...
    },
    {
      ...
      message_templates: {
        name: "Chuỗi bài học Vận hành Spa Bài 1",
        channel: "email_campaign",
  ...
  const loadAllData = async () => {
    ...
    if (useLocalFallback) {
      ...
      setTemplates([
        {
          ...
          channel: "email_campaign",
          purpose: "marketing_campaign",
          body_template: "Kính gửi quý Đối tác,...",
          requires_opt_in: true,
          include_unsubscribe: true,
          max_send_frequency_days: 30,
          banner_image_url: "https://picsum.photos/600/200",
          cta_label: "Đăng ký Slot",
          cta_url: "https://desembrevn.com/register",
        },
        {
          id: "tpl-2",
          name: "Mẫu thông báo Chính sách Đại lý",
          channel: "email_campaign",
          purpose: "monthly_campaign",
          body_template: "Thông tin chiết khấu...",
      ...
      setZnsTemplates([
        {
          id: "zns-tpl-1",
          zalo_template_id: "123456",
          template_name: "🔔 Xác nhận Đơn hàng Mỹ phẩm",
          category: "transactional",
          purpose: "order_status",
          required_params: ["customer_name", "order_id"],
          sample_payload: { customer_name: "Spa Lan Vy", order_id: "DH-1002" },
          is_active: true,
          sender_account_id: "snd-zalo",
        },
        {
          id: "zns-tpl-2",
          zalo_template_id: "789012",
          template_name: "💎 Thư mời Hội thảo Chuyển giao Phác đồ",
          category: "transactional",
          purpose: "event_invitation",
          required_params: ["customer_name", "event_name", "event_time"],
          sample_payload: {
            customer_name: "Spa Lan Vy",
            event_name: "Chuyển Giao Vy Tảo C",
            event_time: "09:00 - 30/05",
          },
          is_active: true,
          sender_account_id: "snd-zalo",
        },
      ...
      setSenders([
        ...
        {
          id: "snd-zalo",
          name: "Zalo OA DESEMBRE Official",
          sender_email: "oa@desembrevn.com",
          channel: "zalo_oa",
          is_active: true,
          health_status: "healthy",
          daily_usage: 150,
    ...
    try {
      // 1. Tải chiến dịch
      let queryCamps = supabase.from("marketing_campaigns").select(`
          *,
      ...
      const { data: cData, error: cErr } = await queryCamps.order("created_at", {
        ascending: false,
      });
      ...
      const { data: znsData } = await supabase
        .from("zns_templates")
        .select("*")
        .eq("is_active", true)
        .eq("status", "approved");
      ...
      if (isSale && !isAdmin && !isSubAdmin) {
        ...
        const { data: personalAccs } = await supabase
          ...
          .eq("is_active", true);
        if (personalAccs) {
          availableSenders = personalAccs
            .filter((a: any) => !!a.provider_secret)
            .map((a: any) => ({
              id: a.id,
              name: a.account_name,
      ...
      const { data: custData } = await supabase
        .from("customers")
        .select("id, email, phone, marketing_opt_in, marketing_opt_out_at, last_marketing_sent_at");
  ...
  useEffect(() => {
    ...
    const firstEmailSender =
      senders.find((s) => s.channel?.toLowerCase().includes("email"))?.id || senders[0]?.id || "";
  ...
  const handleOpenWizard = () => {
    ...
    const firstZnsSender =
      senders.find((s) => s.channel === "zalo_oa" || s.channel === "zalo")?.id ||
      senders[0]?.id ||
      "";
  ...
  const isSelectedSenderAllowed = useMemo(() => {
    ...
    const resolution = resolveSenderForMessage({
      channel:
        campaignType === "zns"
          ? "zalo_oa"
          : (selectedTemplate?.channel || "email").toLowerCase().includes("email")
            ? "email"
            : "zalo_oa",
      mode: "campaign",
  ...
  const handleDispatchCampaign = async () => {
    ...
    if (useLocalFallback) {
      setTimeout(() => {
        const localCamps = JSON.parse(localStorage.getItem("mock_campaigns") || "[]");
        const createdObj: Campaign = {
          ...
          message_templates:
            campaignType === "general"
              ? {
                  name: selectedTemplate?.name || "Mẫu tùy chỉnh",
                  channel: selectedTemplate?.channel || "email",
                  purpose: selectedTemplate?.purpose || "marketing",
                }
              : undefined,
          zns_templates:
            campaignType === "zns"
              ? {
                  template_name: selectedTemplate?.name || "Mẫu ZNS",
                  category: "transactional",
          ...
          sender_accounts: {
            id: formSenderId,
            name: senders.find((s: any) => s.id === formSenderId)?.name || "OA Hệ thống",
            sender_email: "oa@desembrevn.com",
          },
          customer_segments: {
            name:
              segments.find((s: any) => s.id === formSegmentId)?.name || "Tập khách hàng tùy chọn",
          },
        };
    ...
    try {
      ...
      const snapshotInserts = fullCusts.map((c) => {
        const isBlocked =
          ...
          (campaign.zns_template_id && c.marketing_opt_in === false);
  ...
  const startSendingCampaign = async (campaignId: string) => {
    ...
    const total =
      selectedCampaign?.metrics?.total_targets || selectedCampaign?.estimated_recipients || 0;
    if (total === 0) {
      toast.error(
        "Chiến dịch bắt buộc phải có danh sách người nhận (snapshot) đã đóng băng. Vui lòng duyệt chiến dịch trước khi gửi.",
      );
      return;
    }
    ...
    if (useLocalFallback) {
      ...
      const mockLoop = setInterval(() => {
        if ((window as any).stopZnsCampaignSend) {
          ...
          setConsoleLogs((prev) => [
            ...prev,
            `[${new Date().toLocaleTimeString()}] Đã tạm dừng gửi.`,
          ]);
          return;
        }
        ...
        setConsoleLogs((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Gửi thành công lô mock (10/10) - Tiến trình: ${currentProcessed}%`,
        ]);
        ...
        if (currentProcessed >= 100) {
          ...
          setConsoleLogs((prev) => [
            ...prev,
            `[${new Date().toLocaleTimeString()}] Chiến dịch hoàn thành! 🎉`,
          ]);
    ...
    try {
      ...
      while (!finished && lastStatus === "sending") {
        if ((window as any).stopZnsCampaignSend) {
          setConsoleLogs((prev) => [
            ...prev,
            `[${new Date().toLocaleTimeString()}] Phát hiện yêu cầu tạm dừng từ Admin.`,
          ]);
          break;
        }
        ...
        setConsoleLogs((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Đang gọi process-zns-campaign gửi lô 30 tin tiếp theo...`,
        ]);
        ...
        if (data.success) {
          ...
          if (data.paused) {
            setConsoleLogs((prev) => [
              ...prev,
              `[TẠM DỪNG] Tự động tạm dừng do lỗi sender: ${data.error}`,
            ]);
      ...
      if (finished) {
        setConsoleLogs((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Chiến dịch gửi hoàn tất vĩnh viễn! 🎉`,
        ]);
      }
    } catch (err: any) {
  ...
  const processSingleBatch = async (campaignId: string) => {
    ...
    setConsoleLogs((prev) => [
      ...prev,
      `[${new Date().toLocaleTimeString()}] Thực hiện gửi thủ công 1 lô tiếp theo (Tối đa 30 tin)...`,
    ]);
    ...
    if (useLocalFallback) {
      ...
      if (idx !== -1) {
        ...
        setConsoleLogs((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Gửi thành công lô thủ công mock (10/10) - Tiến trình: ${currentProcessed}%`,
        ]);
        if (currentProcessed >= 100) {
          toast.success("Chiến dịch hoàn thành gửi (Sandbox)");
          setConsoleLogs((prev) => [
    ...
    try {
      ...
      if (data.success) {
        ...
        const { data: updatedCamp } = await supabase
          .from("marketing_campaigns")
          .select(
            "*, message_templates(*), zns_templates(*), sender_accounts(*), customer_segments(*)",
          )
          .eq("id", campaignId)
          .single();
        if (updatedCamp) setSelectedCampaign(updatedCamp);
  ...
  const handleMockSend = async () => {
    ...
    setConsoleLogs((prev) => [
      ...prev,
      `[${new Date().toLocaleTimeString()}] Bắt đầu quy trình Mock Production Send...`,
    ]);
    try {
      const {
        data: { session },
      ...
      setConsoleLogs((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Đã lưu chữ ký xác nhận của Admin.`,
      ]);

      // 2. Kích hoạt Edge Function process-marketing-campaign (Mock mode)
      setConsoleLogs((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Gọi Edge Function process-marketing-campaign...`,
      ]);
      ...
      setConsoleLogs((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] HOÀN TẤT MOCK SEND! Result: ${JSON.stringify(data)}`,
      ]);
      toast.success("Đã chạy Mock Sender thành công.");

      // Update local state to reflect changes
      setCampaigns((prev) =>
        prev.map((c) => (c.id === selectedCampaign.id ? { ...c, status: "completed" } : c)),
      );
      ...
      setConsoleLogs((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] LỖI: ${err.message}`,
      ]);
            ...
            {loading ? (
              ...
              filteredCampaigns.map((c: any) => {
                ...
                const progressPercent =
                  totalTargets > 0
                    ? Math.min(Math.round((sentCount / totalTargets) * 100), 100)
                    : 0;

                const templateName = c.zns_template_id
                  ? c.zns_templates?.template_name
                  : c.message_templates?.name;
                const channelLabel = c.zns_template_id
                  ? "ZNS"
          ...
          <div className="bg-slate-900 px-6 py-4 border-b border-slate-800 flex items-center justify-between">
            <div>
              ...
              <DialogTitle className="text-base font-black text-white">
                Khởi tạo Chiến dịch Tiếp thị Mới
              </DialogTitle>
            </div>
            <div className="flex items-center gap-1.5">
              {[1, 2, 3, 4].map((st) => (
                <div key={st} className="flex items-center">
                  ...
                  {st < 4 && (
                    <span
                      className={`w-4 h-0.5 mx-0.5 ${wizardStep > st ? "bg-purple-500/50" : "bg-slate-800"}`}
                    />
                  )}
                </div>
              ))}
          ...
          <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="space-y-1.5">
                  ...
                  <span className="text-[10px] text-slate-500 block">
                    Sử dụng tên tường minh để tra cứu hiệu quả gửi về sau.
                  </span>
                </div>

                <div className="space-y-1.5 pb-1">
                  <Label className="text-xs font-bold text-slate-300">
                    Loại Chiến dịch Tiếp thị
                  </Label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFormZnsTemplateId(znsTemplates[0]?.id || "");
                        const firstZnsSender =
                          senders.find((s) => s.channel === "zalo_oa" || s.channel === "zalo")
                            ?.id || "";
                        setFormSenderId(firstZnsSender);
                      }}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFormTemplateId(templates[0]?.id || "");
                        const firstEmailSender =
                          senders.find((s) => s.channel === "email")?.id || "";
                        setFormSenderId(firstEmailSender);
                      }}
                      className={`h-10 rounded-xl text-xs font-bold transition-all border ${
                        campaignType === "general"
                ...
                {campaignType === "zns" ? (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-bold text-slate-300">
                      Mẫu Tin ZNS Đã Đăng Ký (Approved ZNS Templates) *
                    </Label>
                    <select
                      ...
                      {znsTemplates.length === 0 ? (
                        ...
                        znsTemplates.map((t) => (
                          <option key={t.id} value={t.id}>
                            [{t.category.toUpperCase()}] — {t.template_name} (OA:{" "}
                            {senders.find((s) => s.id === t.sender_account_id)?.name || "OA"})
                          </option>
                        ))
                      )}
                    </select>
                ...
                {selectedTemplate && (
                  <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800 space-y-2 text-xs">
                    <span className="text-[10px] font-bold text-purple-400 block uppercase">
                      Thuộc tính Mẫu đang chọn:
                    </span>
                    <div className="grid grid-cols-2 gap-2 text-[11px]">
                      <div>
                        • Phân loại:{" "}
                        <strong className="text-slate-300">{selectedTemplate.purpose}</strong>
                      </div>
                      <div>
                        • Kênh:{" "}
                        <strong className="text-slate-300">{selectedTemplate.channel}</strong>
                      </div>
                      <div>
                        • Bắt buộc Opt-in:{" "}
                        <strong
                          className={
                            selectedTemplate.requires_opt_in ? "text-amber-400" : "text-slate-500"
                          }
                        >
                          {selectedTemplate.requires_opt_in ? "Có" : "Không"}
                        </strong>
                      </div>
                      <div>
                        • Giới hạn chu kỳ:{" "}
                        <strong className="text-purple-400">
                          {selectedTemplate.max_send_frequency_days
                            ? `${selectedTemplate.max_send_frequency_days} ngày`
                            : "Không giới hạn"}
                        </strong>
                      </div>
                    </div>
            ...
            {wizardStep === 2 && (
              <div className="space-y-4 animate-fade-in">
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold text-slate-300">
                    Tài khoản Nguồn Phát hành (Sender Account) *
                  </Label>
            ...
            {wizardStep === 3 && (
              <div className="space-y-4 animate-fade-in">
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold text-slate-300">
                    Tập Khách hàng Đích (Target Audience Segment) *
                  </Label>
                  <select
                    value={formSegmentId}
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center">
                      ...
                      {campaignType === "zns" ? (
                        <>
                          ...
                          <span className="text-sm font-bold text-amber-400">
                            {complianceEstimate.missingPhone}
                          </span>
                        </>
                      ) : (
                        <>
                    ...
                    <div className="p-2 rounded-lg bg-slate-900/80">
                      ...
                      <span className="text-sm font-bold text-rose-400">
                        {complianceEstimate.optOut}
                      </span>
                    </div>
                  </div>

                  <p className="text-[10px] text-slate-400 leading-relaxed italic">
                    💡 Engine tự động gạt bỏ các Spa đã từ chối nhận tin hoặc vừa nhận thông điệp
                    cùng mục đích trong chu kỳ giới hạn. Hệ thống đảm bảo an toàn tuyệt đối cho uy
                    tín tên miền gửi đi.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-5 animate-fade-in">
                ...
                <div className="border border-slate-800 rounded-xl overflow-hidden bg-white text-slate-900">
                  <div className="bg-slate-100 p-2 border-b border-slate-200 text-[10px] font-bold text-slate-500 flex items-center justify-between">
                    <span>
                      {campaignType === "zns"
                        ? "Live ZNS Content Preview"
                        : "Live Email Content Preview"}
                    </span>
                    <span className="text-purple-600 font-mono">
                      {campaignType === "zns" ? "Chế độ Tin ZNS B2B" : "Chế độ Chiến dịch B2B"}
                    </span>
                  </div>
                  ...
                  {selectedTemplate?.banner_image_url && (
                    <div className="w-full h-28 bg-slate-100 overflow-hidden">
                      <img
                        src={selectedTemplate.banner_image_url}
                        alt="Banner"
                  ...
                  <div className="p-3 space-y-2 text-xs">
                    <div className="font-bold text-purple-950 border-b pb-1">
                      {campaignType === "zns"
                        ? `Mẫu ZNS: ${selectedTemplate?.name}`
                        : selectedTemplate?.subject_template ||
                          "[DESEMBRE] Thông tin Chương trình Đào tạo Chuyên sâu"}
                    </div>
                    ...
                    <div className="pt-2 border-t mt-2 text-[9px] text-slate-400 italic">
                      {campaignType === "zns" ? (
                        <span>
                          Hệ thống tự động gửi qua Zalo OA chính thức của doanh nghiệp sau khi được
                          phê duyệt.
                        </span>
                      ) : (
                        <span>
                          Hệ thống tự động chèn liên kết:{" "}
                          <span className="underline text-purple-600 cursor-pointer">
                            Hủy đăng ký nhận bản tin (Unsubscribe)
                          </span>{" "}
                          theo chuẩn CAN-SPAM.
                        </span>
                      )}
      ...
      <Dialog open={approvalDialogOpen} onOpenChange={setApprovalDialogOpen}>
        <DialogContent className="sm:max-w-[550px] bg-slate-950 text-slate-100 border-slate-800 p-0 overflow-hidden rounded-3xl shadow-2xl">
          <div className="bg-slate-900 px-6 py-4 border-b border-slate-800">
            ...
            <DialogTitle className="text-base font-black text-white">
              Kiểm duyệt & Phê duyệt Chiến dịch
            </DialogTitle>
          </div>
          ...
          <div className="p-6 space-y-6">
            <div className="space-y-1.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 p-3 bg-slate-900/50 rounded-xl border border-slate-800 text-[11px]">
                <div>
                  • Kênh:{" "}
                  <strong className="text-purple-400">
                    {selectedCampaign?.zns_template_id ? "Tin ZNS Zalo" : "Email/SMS"}
                  </strong>
                </div>
                <div>
                  • Tài khoản gửi:{" "}
                  <strong className="text-slate-300">
                    {selectedCampaign?.sender_accounts?.name}
                  </strong>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-purple-950/20 border border-purple-900/30 space-y-3">
              ...
              {selectedCampaign?.sender_accounts?.health_status &&
                selectedCampaign.sender_accounts.health_status !== "healthy" && (
                  <div className="p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-[10px] text-rose-400 flex items-center gap-1.5">
                    ...
                    <span>
                      <strong>Cảnh báo:</strong> Tài khoản gửi có trạng thái sức khỏe không tốt (
                      {selectedCampaign.sender_accounts.health_status})!
                    </span>
                  </div>
                )}
            ...
            {isAdminOrSubAdmin ? (
              <div className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800">
                ...
                />
                <label
                  htmlFor="chkConfirmApprove"
                  className="text-[11px] text-slate-300 leading-normal cursor-pointer select-none"
                >
                  Xác nhận phê duyệt chiến dịch này. Hệ thống sẽ tiến hành{" "}
                  <strong>đóng băng (snapshot) danh sách người nhận</strong> tại thời điểm này để
                  chuẩn bị gửi.
                </label>
              </div>
            ) : (
              <div className="p-3.5 text-center text-xs rounded-xl bg-rose-500/5 border border-rose-500/10 text-rose-400">
                ⚠️ Tài khoản của bạn không có vai trò Admin/SubAdmin. Bạn không có quyền phê duyệt
                chiến dịch này.
              </div>
            )}
          </div>
            ...
            <div className="md:col-span-7 p-6 overflow-y-auto space-y-5 flex flex-col h-full">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-[11px] font-medium text-slate-300">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={selectedCampaign?.sender_account_id ? "text-emerald-400" : ""}>
                      Đã chọn sender
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={
                        selectedCampaign?.approval_status === "approved" ? "text-emerald-400" : ""
                      }
                    >
                      Campaign đã duyệt
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    {readinessData?.approved ? (
                      <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                    ) : (
                      <div className="w-4 h-4 rounded-full border-2 border-slate-700" />
                    )}
                    <span className={readinessData?.approved ? "text-emerald-400" : ""}>
                      Readiness pass
                    </span>
                  </div>
                </div>
                {!selectedCampaign?.sender_account_id && (
                  <div className="mt-3 p-2 bg-amber-500/10 border border-amber-500/20 rounded flex gap-2">
                    ...
                    <span className="text-[10px] text-amber-400">
                      Đang dùng cấu hình hệ thống mặc định vì campaign chưa chọn sender.
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400 font-bold uppercase block tracking-wider">
                    Tiến trình Gửi
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wide border ${
                ...
                {(() => {
                  const total =
                    selectedCampaign?.metrics?.total_targets ||
                    selectedCampaign?.estimated_recipients ||
                    0;
                  const sent =
                    selectedCampaign?.successful_recipients || selectedCampaign?.metrics?.sent || 0;
                  const failed =
                    selectedCampaign?.failed_recipients || selectedCampaign?.metrics?.failed || 0;
              ...
              <div className="flex-1 flex flex-col min-h-[160px] bg-black rounded-xl border border-slate-800 p-3 font-mono text-[10px] text-slate-300 overflow-hidden relative">
                <div className="absolute top-1 right-2 text-[9px] text-slate-600 select-none uppercase font-sans">
                  Edge Function Terminal
                </div>
                <div className="flex-1 overflow-y-auto space-y-1 select-text scrollbar-thin">
                  {consoleLogs.length === 0 ? (
                    <span className="text-slate-600 italic">
                      Sẵn sàng kích hoạt operational batch sending... Click "Bắt đầu gửi" để khởi
                      chạy lô đầu tiên.
                    </span>
                  ) : (
              ...
              {readinessData && (
                <div className="mt-4 p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${readinessData.approved ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                    >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <div className="p-3 rounded-lg bg-slate-950 border border-slate-800">
                      ...
                      <strong
                        className={`text-lg ${readinessData.rate_limit?.exceeds_limit ? "text-amber-400" : "text-emerald-400"}`}
                      >
                      ...
                      <div className="p-2 rounded bg-slate-950 border border-slate-800 text-[11px] space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="text-slate-300 font-mono">
                            {readinessData.rate_limit?.max_batch_size}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span
                            className={
                              readinessData.rate_limit?.exceeds_limit
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="p-2 rounded bg-slate-950 border border-slate-800 text-[11px] space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span
                            className={
                              readinessData.production_sending_enabled
                  ...
                  <div className="space-y-2">
                    <div className="text-xs font-bold text-slate-300">
                      Preview Recipients (Max {readinessData.preview_limit})
                    </div>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      {readinessData.preview_recipients?.length > 0 ? (
                        readinessData.preview_recipients.map((r: any, idx: number) => (
                          <div
                            key={idx}
                            className="text-[10px] text-slate-400 p-1.5 bg-slate-950 rounded flex justify-between"
                          >
                            <span>{r.name}</span>
                  ...
                  <div className="p-3 bg-rose-500/5 border border-rose-500/20 rounded-lg space-y-3">
                        ...
                        {readinessData.provider_mode === "resend_pilot" ? (
                          <>
                            <p className="text-[10px] text-slate-400 mt-1">
                              Để chạy Pilot Send, bạn phải gõ chữ{" "}
                              <strong className="text-white">CONFIRM</strong> vào ô bên dưới.
                              <br />
                              <strong className="text-amber-400">
                                Email thật sẽ được gửi, nhưng chỉ tới whitelist nội bộ. Không gửi
                                tới audience khách hàng.
                              </strong>
                            </p>
                            <p className="text-[9px] text-rose-400 mt-1">
                              Sau pilot, hiển thị reminder: Tắt
                              MARKETING_PRODUCTION_SENDING_ENABLED=false và chuyển
                              MARKETING_PROVIDER_MODE=mock.
                            </p>
                          </>
                        ) : (
                          <p className="text-[10px] text-slate-400 mt-1">
                            Để chạy Mock Production Send, bạn phải gõ chữ{" "}
                            <strong className="text-white">CONFIRM</strong> vào ô bên dưới. Việc này
                            sẽ lưu chữ ký số của bạn vào hệ thống.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        {readinessData.provider_mode === "resend_pilot"
                          ? "Run Email Pilot Send (Internal Only)"
                          : "Run Mock Production Send"}
                      </Button>
                    </div>
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-slate-800/60 space-y-4">
                  ...
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {selectedCampaign?.approval_status === "approved" ? (
                        ...
                        <Button
                          disabled
                          className="h-8 bg-slate-800 text-slate-500 font-bold text-[10px] cursor-not-allowed"
                        >
                    ...
                    {!selectedCampaign?.sender_account_id && (
                      <div className="bg-amber-500/10 border border-amber-500/20 p-2 rounded-lg mb-2">
                        <p className="text-[10px] text-amber-500 font-bold">
                          ⚠️ Vui lòng chọn Sender Account bên trái để có thể gửi thử.
                        </p>
                      </div>
                    )}
                    <div className="flex flex-col gap-2">
                      {isAdminOrSubAdmin && (
                        <Button
                          onClick={() => {
                            ...
                            if (
                              selectedCampaign?.channel === "zalo" ||
                              selectedCampaign?.channel === "zalo_oa"
                            ) {
                          ...
                          disabled={
                            !selectedCampaign?.sender_account_id ||
                            selectedCampaign?.sender_accounts?.status === "archived" ||
                            selectedCampaign?.sender_accounts?.health_status !== "healthy"
                          }
                ...
                <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(!selectedCampaign?.approval_status ||
                      selectedCampaign?.approval_status === "draft") &&
                      canManageCampaign(selectedCampaign) && (
                        <Button
                          onClick={() => handleRequestReview(selectedCampaign?.id || "")}
                          className="h-8 px-3 bg-purple-600 hover:bg-purple-500 text-[10px]"
                        >
                          Gửi Yêu cầu Duyệt
                        </Button>
                      )}
                    {selectedCampaign?.approval_status === "pending_approval" &&
                      isAdminOrSubAdmin && (
                        <Button
                          ...
                          className="h-8 px-3 bg-emerald-600 hover:bg-emerald-500 text-[10px]"
                        >
                          Duyệt
                        </Button>
                      )}
                    ...
                    <Button
                      ...
                      className="h-8 px-3 bg-transparent border-slate-700 text-[10px] text-slate-400"
                    >
                      Đóng
                    </Button>
                  </div>
            ...
            <div className="md:col-span-5 p-5 overflow-hidden flex flex-col h-[55vh] md:h-full max-h-[70vh]">
              ...
              {snapshotLoading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 border border-dashed border-slate-800/80 rounded-2xl bg-slate-900/10 text-slate-600 text-center">
                  ...
                  <span className="text-[10px] mt-0.5 leading-relaxed">
                    Danh sách sẽ được đóng băng snapshot cố định ngay khi chiến dịch được Admin phê
                    duyệt.
                  </span>
                </div>
              ) : (
                <div className="flex-1 overflow-y-auto space-y-2.5 scrollbar-thin select-none pr-1">
                  {snapshots.map((snap) => {
                    ...
                    return (
                      <div
                        key={snap.id}
                        className="p-3 rounded-xl bg-slate-900/50 border border-slate-900/80 flex items-center justify-between gap-2 text-xs"
                      >
                        <div className="space-y-0.5">
                          <strong className="text-slate-200 block text-[11px] truncate max-w-[150px]">
                            {name}
                          </strong>
                        ...
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${getSnapStatusClass(snap.status)}`}
                        >
        ...
        <DialogContent className="sm:max-w-[450px] bg-white text-slate-900 p-6 rounded-3xl shadow-2xl">
          ...
          <div className="space-y-4">
            <div className="bg-amber-500/10 border border-amber-500/20 text-amber-400 p-3 rounded-xl text-[11px] font-medium flex items-start gap-2 leading-relaxed">
              ...
              <span>
                Đây là sandbox test. Email chỉ gửi tới địa chỉ nằm trong whitelist. Không gửi tới
                audience thật. Cố tình nhập sai hoặc gửi ra ngoài whitelist sẽ bị chặn.
              </span>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300">
                Email nhận Test (Whitelist Only):
              </label>
              <Input
                type="email"
        ...
        <DialogContent className="sm:max-w-[450px] bg-white text-slate-900 p-6 rounded-3xl shadow-2xl">
          ...
          <div className="space-y-4">
            <div className="bg-amber-500/10 border border-amber-500/20 text-amber-400 p-3 rounded-xl text-[11px] font-medium flex items-start gap-2 leading-relaxed">
              <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <span>
                Đây là Zalo OA sandbox test. Tin chỉ gửi tới zalo_user_id whitelist. Không gửi qua
                Phone. Không gửi tới audience thật.
              </span>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300">
                Zalo User ID nhận Test (Whitelist Only):
              </label>
              <Input
                type="text"
