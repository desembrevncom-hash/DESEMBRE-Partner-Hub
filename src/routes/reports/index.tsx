  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const funnel = SALES_PIPELINE_STAGES.map((stage) => ({
        name: stage.label,
        count:
          customers?.filter((c) => mapLegacyStageToNew(c.lifecycle_stage) === stage.value).length ||
          0,
      }));
      ...
      setStats({
        ...
        conversionRate: customers?.length
          ? Math.round(
              (customers.filter((c) => mapLegacyStageToNew(c.lifecycle_stage) === "purchased")
                .length /
                customers.length) *
                100,
            )
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <CardHeader className="p-8 pb-0 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium mt-1">
                  Biến động doanh số 30 ngày gần nhất
                </p>
              </div>
              <Badge
                variant="secondary"
                className="bg-slate-50 text-slate-500 rounded-lg px-3 py-1 font-bold"
              >
                LIVE
              </Badge>
            </CardHeader>
          ...
          <Card className="rounded-[32px] border-none shadow-sm overflow-hidden bg-white">
            <CardHeader className="p-8 pb-0">
              ...
              <p className="text-xs text-slate-400 font-medium mt-1">
                Top 5 tỉnh thành nhiều khách hàng nhất
              </p>
            </CardHeader>
            <CardContent className="p-8">
          ...
          <Card className="lg:col-span-2 rounded-[32px] border-none shadow-sm overflow-hidden bg-white">
            <CardHeader className="p-8 pb-0">
              ...
              <p className="text-xs text-slate-400 font-medium mt-1">
                Phân bổ khách hàng qua 12 giai đoạn Pipeline
              </p>
            </CardHeader>
            <CardContent className="p-8">
            ...
            <CardContent className="p-8 space-y-6">
              {[
                ...
                <div key={staff.name} className="flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black border transition-all group-hover:scale-105 ${idx === 0 ? "bg-indigo-50 text-indigo-600 border-indigo-100" : "bg-slate-50 text-slate-400 border-slate-100"}`}
                    >
                      {staff.avatar}
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[9px] border-emerald-100 text-emerald-600 bg-emerald-50"
                    >
                      +5.2%
                    </Badge>
                  </div>
              ...
              <Button
                variant="ghost"
                className="w-full rounded-xl text-xs font-bold text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 mt-4"
              >
    ...
    <Card className="rounded-[28px] border-none shadow-sm overflow-hidden bg-white hover:shadow-md transition-all group">
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div
            className={`flex items-center gap-1 text-[10px] font-black px-2 py-1 rounded-lg ${isUp ? "text-emerald-600 bg-emerald-50" : "text-red-500 bg-red-50"}`}
          >
        ...
        <div className="flex items-baseline gap-1 mt-1">
          ...
          {subValue && (
            <span className="text-xs font-bold text-slate-400 uppercase">{subValue}</span>
          )}
        </div>
      </CardContent>
    </Card>
