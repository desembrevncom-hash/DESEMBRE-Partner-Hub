...
function AdminReclamationPage() {
  ...
  const [selectedCareModel, setSelectedCareModel] = useState<
    "tele_owned" | "tele_qualified_then_sale"
  >("tele_owned");

  // Reclaim modal state
  const [reclaimCustomer, setReclaimCustomer] = useState<any | null>(null);
  const [reclaimReason, setReclaimReason] = useState<string>(
    "Quá hạn tương tác chăm sóc, thu hồi về kho tự do.",
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 2. Fetch profiles
      const { data: profData } = await supabase.from("profiles").select("id, display_name, email");
  ...
  const handleScanReclamation = async () => {
    ...
    const loadingToastId = toast.loading(
      "Bắt đầu quét danh sách khách hàng cần cảnh báo/thu hồi...",
    );
    try {
  ...
  const handleExtend3Days = async (customer: any) => {
    try {
      ...
      const newInteractionTime = new Date(
        currentLastInteraction.getTime() + 3 * 24 * 60 * 60 * 1000,
      ).toISOString();
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          at_risk_at: customer.at_risk_at
            ? new Date(
                new Date(customer.at_risk_at).getTime() + 3 * 24 * 60 * 60 * 1000,
              ).toISOString()
            : null,
          reclaimable_at: customer.reclaimable_at
            ? new Date(
                new Date(customer.reclaimable_at).getTime() + 3 * 24 * 60 * 60 * 1000,
              ).toISOString()
            : null,
  ...
  const handleConfirmReassign = async () => {
    ...
    try {
      ...
      await supabase.from("customer_activities").insert({
        ...
        title:
          reassignType === "sale"
            ? "Chia lại khách hàng cho Sale"
            : "Chia lại khách hàng cho Trưởng Tele",
        content:
          reassignType === "sale"
  ...
  const atRiskCustomers = useMemo(
    () => filteredCustomers.filter((c) => c.ownership_status === "at_risk"),
    [filteredCustomers],
  );
  const reclaimableCustomers = useMemo(
    () => filteredCustomers.filter((c) => c.ownership_status === "reclaimable"),
    [filteredCustomers],
  );
  const freePoolCustomers = useMemo(
    () => filteredCustomers.filter((c) => c.ownership_status === "free_pool"),
    [filteredCustomers],
  );
            ...
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-black text-slate-900 tracking-tighter mt-1">
                  {customers.filter((c) => c.ownership_status === "at_risk").length}
                </h3>
              </div>
          ...
          <Tabs defaultValue="at_risk" className="w-full">
            <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-4">
              <TabsList className="bg-slate-100 p-1 h-auto rounded-xl flex w-fit gap-2">
                <TabsTrigger
                  value="at_risk"
                  className="rounded-lg text-xs font-black px-4 py-2 uppercase"
                >
      ...
      <Dialog open={!!reclaimCustomer} onOpenChange={(o) => !o && setReclaimCustomer(null)}>
        <DialogContent className="sm:max-w-[425px] rounded-[32px] border-none shadow-2xl p-8">
          ...
          <DialogFooter className="gap-2">
            <Button
              variant="ghost"
              onClick={() => setReclaimCustomer(null)}
          ...
          <div className="py-6 space-y-4">
            ...
            {reassignType === "tele" && (
              <div className="space-y-2 pt-2">
                ...
                <Select
                  value={selectedCareModel}
                  onValueChange={(val: any) => setSelectedCareModel(val)}
                >
                  <SelectTrigger className="h-12 rounded-2xl border-slate-200">
        ...
        <tbody className="divide-y divide-slate-50">
          {items.map((c) => {
            ...
            return (
              <tr
                key={c.id}
                className="hover:bg-slate-50/50 transition-all font-medium text-slate-700"
              >
                  ...
                  <div className="space-y-1">
                    {saleName && (
                      <p className="text-[10px] font-semibold text-indigo-600 flex items-center gap-1">
                        <span className="font-black bg-indigo-50 px-1.5 py-0.5 rounded text-[8px] uppercase">
                          Sale
                        </span>{" "}
                        {saleName}
                      </p>
                    ...
                    {teleName && (
                      <p className="text-[10px] font-semibold text-amber-600 flex items-center gap-1">
                        <span className="font-black bg-amber-50 px-1.5 py-0.5 rounded text-[8px] uppercase">
                          Tele
                        </span>{" "}
                        {teleName}
                      </p>
                ...
                <td className="px-8 py-5 text-center">
                  <Badge
                    className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 ${getLifecycleStageBadge(c.lifecycle_stage)}`}
                  >
                ...
                <td className="px-8 py-5 text-xs">
                  <div>
                    <p className="text-slate-600 font-semibold">
                      {formatDate(c.last_owner_activity_at || c.last_contacted_at)}
                    </p>
                ...
                <td className="px-8 py-5 text-xs font-semibold text-slate-500">
                  {c.last_order_at
                    ? format(new Date(c.last_order_at), "dd/MM/yyyy")
                    : "Chưa lên đơn"}
                </td>
                <td className="px-8 py-5 text-xs">
                  <p className="text-red-700 font-bold max-w-xs leading-normal">
                    {c.reclaim_reason || "Lâu ngày chưa chăm sóc."}
                  </p>
                </td>
