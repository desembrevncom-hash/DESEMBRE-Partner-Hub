...
function AdminUsersPage() {
  ...
  const filteredProfiles = profiles.filter(p => {
    const matchSearch = (p.display_name || p.email || "").toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'sale': return <ShoppingBag className="w-3.5 h-3.5" />;
      case 'telesale': return <Phone className="w-3.5 h-3.5" />;
      case 'tele_lead': return <Users className="w-3.5 h-3.5" />;
      ...
      <header className="bg-white/80 border-b border-slate-200/60 sticky top-0 z-30 backdrop-blur-md">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between max-w-7xl">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h1 className="text-lg font-black text-slate-900 tracking-tight">Nhân sự & Phân quyền</h1>
          ...
          <div className="flex items-center gap-3">
             <Button onClick={reload} variant="ghost" size="icon" className="rounded-xl text-slate-400">
           ...
           <div className="lg:col-span-2 space-y-6">
              <Card className="rounded-[32px] border-none shadow-sm overflow-hidden bg-white">
                 <CardHeader className="p-8 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <CardTitle className="text-base font-black text-slate-900 uppercase tracking-widest">Danh sách Đội ngũ</CardTitle>
                    ...
                    <div className="overflow-x-auto">
                       <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-slate-50">
                             {busy ? (
                               <tr><td colSpan={3} className="px-8 py-20 text-center animate-pulse text-slate-300 font-bold uppercase text-[10px]">Đang đồng bộ dữ liệu...</td></tr>
                             ) : filteredProfiles.length === 0 ? (
                               <tr><td colSpan={3} className="px-8 py-20 text-center text-slate-300 font-bold uppercase text-[10px]">Không tìm thấy nhân sự</td></tr>
                                      ...
                                      <td className="px-8 py-5">
                                         <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 border border-slate-200">
                                               {profile.display_name?.slice(0,1) || profile.email?.slice(0,1)}
                                            </div>
                                            <div>
                                               <p className="text-sm font-black text-slate-900">{profile.display_name || "Chưa đặt tên"}</p>
                                               <p className="text-[10px] font-bold text-slate-400">{profile.email}</p>
                                            </div>
                                         </div>
                                      </td>
                                      <td className="px-8 py-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <Card className="rounded-[24px] border-none shadow-sm bg-slate-900 text-white p-6">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed font-medium">
                       Quyền **Admin** cho phép truy cập toàn bộ dữ liệu tài chính và báo cáo. Hãy cẩn thận khi cấp quyền này cho nhân sự mới. Quyền **Sale** và **Telesale** sẽ giới hạn dữ liệu theo từng cá nhân phụ trách.
                    </p>
                 </Card>
                 <Card className="rounded-[24px] border-none shadow-sm bg-white p-6 border border-slate-100">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed font-medium">
                       Hệ thống sử dụng cơ chế đồng bộ thời gian thực. Mọi thay đổi về phân quyền sẽ có hiệu lực ngay lập tức khi nhân viên tải lại trang hoặc thực hiện thao tác mới.
                    </p>
                 </Card>
              </div>
           </div>
      ...
      <Dialog open={!!deleteCandidate} onOpenChange={(open) => !open && setDeleteCandidate(null)}>
        <DialogContent className="sm:max-w-[425px] rounded-[32px] border-none shadow-2xl p-8">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm pt-4 leading-relaxed font-medium text-slate-500">
              Bạn đang thực hiện thao tác xóa vĩnh viễn tài khoản <strong className="text-slate-900 underline decoration-rose-500 decoration-2">{deleteCandidate?.display_name || deleteCandidate?.email}</strong>. Hành động này không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <div className="py-6">
          ...
          <DialogFooter className="gap-3">
            <Button variant="ghost" onClick={() => setDeleteCandidate(null)} className="rounded-xl font-bold text-slate-400">Hủy bỏ</Button>
            <Button
              variant="destructive"
