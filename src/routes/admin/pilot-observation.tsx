...
function PilotObservationDashboard() {
  ...
  const dragSuccessRate = totalDrags.length
    ? Math.round((dragSuccess / totalDrags.length) * 100)
    : 0;

  const quickLogs = metrics.filter((m) => m.action_key === "quick_log").length;
  const searches = metrics.filter((m) => m.action_key === "search").length;

  const filterCounts = metrics
    .filter((m) => m.action_key === "filter_apply")
    .reduce((acc: any, curr) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-[24px] p-5 border border-slate-100 shadow-sm flex flex-col justify-between">
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">
                Trên tổng {totalDrags.length} lượt kéo
              </div>
            </div>
            ...
            <div className="bg-white rounded-[24px] p-5 border border-slate-100 shadow-sm flex flex-col justify-between">
              ...
              <div className="flex gap-2 text-sm font-bold mt-2">
                <span className="text-rose-500 bg-rose-50 px-2 rounded-lg">
                  {bugFeedbacks} Bugs
                </span>
                <span className="text-amber-500 bg-amber-50 px-2 rounded-lg">
                  {slowFeedbacks} Chậm
                </span>
                <span className="text-slate-500 bg-slate-100 px-2 rounded-lg">
                  {confusingFeedbacks} Rối
                </span>
              </div>
              <div className="text-xs text-slate-400 font-medium mt-auto">
                Tổng: {feedbacks.length} feedbacks
              </div>
            </div>
          </div>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        ...
                        {f.feedback_type === "slow" && (
                          <span className="text-[10px] font-black text-amber-600 bg-amber-100 px-2 py-1 rounded-lg uppercase">
                            Chậm
                          </span>
                        )}
                        {f.feedback_type === "confusing" && (
                          <span className="text-[10px] font-black text-slate-600 bg-slate-200 px-2 py-1 rounded-lg uppercase">
                            Rối
                          </span>
                        )}
                        {f.feedback_type === "missing_feature" && (
                          <span className="text-[10px] font-black text-indigo-600 bg-indigo-100 px-2 py-1 rounded-lg uppercase">
                            Thiếu tính năng
                          </span>
                        )}
                    ...
                    {f.page_key && (
                      <p className="text-[10px] text-slate-400 font-mono bg-slate-100 px-2 py-1 rounded inline-block mt-2">
                        📍 {f.page_key}
                      </p>
                    )}
                  </div>
