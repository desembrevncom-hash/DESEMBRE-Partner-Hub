...
function InventoryManagementPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const mockProducts = [
        { id: 1, name: 'Desembre Medi Epi Science P.Skin Care Toner', image_url: 'https://picsum.photos/200/200?random=1', categories: { name: 'Dòng Đặc Trị' }, product_variants: [{ size: '200ml', type: 'retail', price: 850000 }, { size: '1000ml', type: 'salon', price: 2450000 }], stock: 45 },
        { id: 2, name: 'Desembre Pure Science Pure E.R Cream Mask', image_url: 'https://picsum.photos/200/200?random=2', categories: { name: 'Mặt Nạ Kem' }, product_variants: [{ size: '200g', type: 'retail', price: 1200000 }], stock: 12 },
        { id: 3, name: 'Desembre Activator EGF Whitening Ampoule', image_url: 'https://picsum.photos/200/200?random=3', categories: { name: 'Ampoule' }, product_variants: [{ size: '7ml x 10', type: 'retail', price: 3200000 }], stock: 0 },
        { id: 4, name: 'Desembre Derma Science Milk Cleanser', image_url: 'https://picsum.photos/200/200?random=4', categories: { name: 'Làm Sạch' }, product_variants: [{ size: '200ml', type: 'retail', price: 650000 }, { size: '1000ml', type: 'salon', price: 1850000 }], stock: 85 },
      ];
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-20 font-sans antialiased">
      ...
      <header className="bg-white/80 border-b border-slate-200 sticky top-0 z-20 backdrop-blur-md">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between max-w-7xl">
          ...
          <div className="flex items-center gap-3">
             <Button variant="ghost" size="icon" className="rounded-xl text-slate-400"><BarChart3 className="w-4 h-4" /></Button>
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           <InventoryStatCard title="Tổng mã hàng (SKU)" value={products.length} icon={Box} color="indigo" />
           <InventoryStatCard title="Sản phẩm sắp hết" value="8" icon={AlertCircle} color="amber" />
           <InventoryStatCard title="Hết hàng (Out of Stock)" value={products.filter(p => p.stock === 0).length} icon={Archive} color="rose" />
           <InventoryStatCard title="Giá trị kho ước tính" value="1.2B" icon={TrendingUp} color="emerald" />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
           {loading ? (
             <div className="col-span-full py-20 text-center space-y-4">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Đang kiểm kê kho hàng...</p>
             </div>
             ...
             <div className="col-span-full py-20 text-center bg-white rounded-[32px] border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-400 mt-1">Hãy thử điều chỉnh bộ lọc hoặc tìm kiếm lại.</p>
             </div>
           ) : filteredProducts.map(product => (
             <Card key={product.id} className="rounded-[32px] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl hover:-translate-y-2 transition-all duration-500 group">
                <div className="relative aspect-square overflow-hidden bg-slate-100">
                   ...
                   <div className="absolute top-4 left-4">
                      <Badge className={`rounded-lg font-black text-[9px] uppercase shadow-sm border-none ${
                        product.stock > 20 ? 'bg-emerald-500 text-white' : 
                   ...
                   <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="secondary" size="icon" className="rounded-xl bg-white/90 backdrop-blur-md shadow-lg h-9 w-9">
                ...
                <CardContent className="p-6 space-y-4">
                   <div>
                      <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1">{product.categories?.name || 'Chưa phân loại'}</p>
                   ...
                   <div className="space-y-2">
                      {product.product_variants?.map((v: any, idx: number) => (
                         <div key={idx} className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100 group-hover:bg-amber-50/50 group-hover:border-amber-100 transition-all">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{v.type} {v.size}</span>
                   ...
                   <div className="pt-2 flex gap-2">
                      <Button variant="outline" className="flex-1 rounded-xl border-slate-100 text-[10px] font-black text-slate-400 hover:text-slate-900">
