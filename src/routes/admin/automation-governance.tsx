...
function AutomationGovernancePage() {
  ...
  const emergencyStop = async () => {
    if (
      !window.confirm(
        "CẢNH BÁO: Hành động này sẽ TẮT hoàn toàn mọi tự động hoá và sinh việc tự động. Bạn chắc chắn chứ?",
      )
    )
      return;
    try {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4 text-center">
        ...
        <p className="text-slate-500 text-sm mt-2">
          Bạn không có quyền truy cập Automation Governance.
        </p>
        <Link
          to="/workspace"
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400 text-sm font-medium max-w-xl leading-relaxed">
                Trung tâm kiểm soát an toàn hệ thống tự động hoá. Quản lý luồng chạy, chống kẹt
                (locks) và giới hạn spam.
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                variant="destructive"
                className="bg-rose-600 hover:bg-rose-700 font-bold shadow-rose-900/50"
              >
            ...
            <div className="space-y-6">
              ...
              <Card className="border-slate-200 shadow-sm bg-white overflow-hidden relative">
                {data.settings.pilot_mode_enabled &&
                  ...
                  !data.settings.due_generator_enabled && (
                ...
                <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-4">
                  <CardTitle className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-emerald-500" /> System Safe Mode & Runtime
                    Status
                  </CardTitle>
                </CardHeader>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Pilot Mode Safe
                      </Label>
                      <p className="text-xs text-slate-500">Giới hạn các tính năng nguy hiểm.</p>
                    </div>
                    <Switch
                      checked={data.settings.pilot_mode_enabled}
                      onCheckedChange={(v) => updateSetting("pilot_mode_enabled", v)}
                      disabled={saving}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        <Bot className="w-3.5 h-3.5 text-indigo-500" /> Automation Rules
                      </Label>
                      <p className="text-xs text-slate-500">Bật/tắt P4 Automation Engine.</p>
                    </div>
                    <Switch
                      checked={data.settings.automation_enabled}
                      onCheckedChange={(v) => updateSetting("automation_enabled", v)}
                      disabled={saving}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        <CalendarClock className="w-3.5 h-3.5 text-amber-500" /> Due Generator
                      </Label>
                      <p className="text-xs text-slate-500">Bật/tắt quét thông báo quá hạn.</p>
                    </div>
                    <Switch
                      checked={data.settings.due_generator_enabled}
                      onCheckedChange={(v) => updateSetting("due_generator_enabled", v)}
                      disabled={saving}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        <Bell className="w-3.5 h-3.5 text-rose-500" /> Notification Center
                      </Label>
                      <p className="text-xs text-slate-500">Cho phép tạo thông báo mới.</p>
                    </div>
                    <Switch
                      checked={data.settings.notification_enabled}
                      onCheckedChange={(v) => updateSetting("notification_enabled", v)}
                      disabled={saving}
                    />
                  </div>
                </CardContent>
                ...
                <CardContent className="pt-6 space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={limits.auto}
                      onChange={(e) =>
                        setLimits((p) => ({ ...p, auto: parseInt(e.target.value) || 0 }))
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-bold text-slate-700">
                      Notifications Created/Ngày
                    </Label>
                    <Input
                      type="number"
                      value={limits.notif}
                      onChange={(e) =>
                        setLimits((p) => ({ ...p, notif: parseInt(e.target.value) || 0 }))
                      }
                    />
                  </div>
                  <Button
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 space-y-3">
                ...
                <div className="space-y-2">
                  <Link
                    to="/admin/automation-rules"
                    className="flex items-center text-sm font-medium text-indigo-700 hover:text-indigo-900"
                  >
                    <ArrowRight className="w-4 h-4 mr-2" /> Automation Rules MVP
                  </Link>
                  <Link
                    to="/admin/crm-health"
                    className="flex items-center text-sm font-medium text-indigo-700 hover:text-indigo-900"
                  >
                    <ArrowRight className="w-4 h-4 mr-2" /> CRM Health
                  </Link>
                </div>
              </div>
            </div>
                ...
                <Card className="border-slate-200 shadow-sm">
                  <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-1">
                    <span className="text-3xl font-black text-slate-800">
                      {data.stats_today?.automation_runs || 0}
                    </span>
                ...
                <Card className="border-slate-200 shadow-sm">
                  <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-1">
                    <span className="text-3xl font-black text-indigo-600">
                      {data.stats_today?.tasks_created || 0}
                    </span>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                      Tasks Created
                    </span>
                ...
                <Card className="border-slate-200 shadow-sm">
                  <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-1">
                    <span className="text-3xl font-black text-rose-600">
                      {data.stats_today?.notifications_created || 0}
                    </span>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                      Notifs Created
                    </span>
                ...
                <Card className="border-rose-200 shadow-sm bg-rose-50/50">
                  <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-1">
                    <span className="text-3xl font-black text-rose-700">
                      {data.stats_today?.failed_runs || 0}
                    </span>
                ...
                <CardContent className="pt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-slate-50 border border-slate-100 rounded p-3">
                    ...
                    <p className="text-xs font-medium text-slate-800 mt-1">
                      {data.last_runtime?.last_automation_run
                        ? new Date(data.last_runtime.last_automation_run).toLocaleString("vi-VN")
                        : "N/A"}
                    </p>
                  </div>
                  <div className="bg-slate-50 border border-slate-100 rounded p-3">
                    <p className="text-[10px] font-bold text-slate-500 uppercase">
                      Last Due Generator Run
                    </p>
                    <p className="text-xs font-medium text-slate-800 mt-1">
                      {data.last_runtime?.last_due_generator_run
                        ? new Date(data.last_runtime.last_due_generator_run).toLocaleString("vi-VN")
                        : "N/A"}
                    </p>
                  </div>
                  <div className="bg-slate-50 border border-slate-100 rounded p-3">
                    ...
                    <p className="text-xs font-medium text-slate-800 mt-1">
                      {data.last_runtime?.last_notification_created
                        ? new Date(data.last_runtime.last_notification_created).toLocaleString(
                            "vi-VN",
                          )
                        : "N/A"}
                    </p>
                ...
                <CardContent className="p-0">
                  {Object.keys(data.active_locks || {}).length === 0 ? (
                    <div className="p-8 text-center text-slate-400 text-xs">
                      Không có tiến trình nào đang bị khóa.
                    </div>
                  ) : (
                        ...
                        <div key={key} className="p-4 flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              Expires: {new Date(lock.expires_at).toLocaleString("vi-VN")}
                            </p>
                          </div>
                          <Button
                            size="sm"
                      ...
                      {data.recent_logs.map((run: any) => (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              {new Date(run.created_at).toLocaleString("vi-VN")} &bull; Match:{" "}
                              {run.matched_records || run.matched_count} &bull; Act:{" "}
                              {run.actions_taken || run.action_count}
                            </p>
                          </div>
                          {run.error_message && (
                            <div
                              className="max-w-[200px] text-[10px] text-rose-600 bg-rose-50 p-2 rounded truncate"
