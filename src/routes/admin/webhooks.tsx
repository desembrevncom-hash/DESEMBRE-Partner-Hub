...
function WebhookInbox() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (search) {
        query = query.or(`event_type.ilike.%${search}%,dedupe_key.ilike.%${search}%,related_message_id.ilike.%${search}%`);
      }
  ...
  if (!isAdmin && !isSubAdmin) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4 text-center">
        ...
        <p className="text-slate-500 text-sm max-w-sm mt-2">Trang Webhook Inbox chỉ dành cho Admin.</p>
    ...
    <div className="min-h-screen bg-[#f8fafc] p-6 font-sans antialiased">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Nơi tập kết mọi sự kiện Webhook từ Resend & Zalo trước khi xử lý Logic.</p>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-amber-700 text-xs mt-1 leading-relaxed">
              Webhook Inbox hiện chỉ là chế độ quan sát. Hệ thống chưa tự động cập nhật Suppression, Delivery Logs hoặc Automation từ webhook. Không có dữ liệu khách hàng nào bị ảnh hưởng.
            </p>
          </div>
        </div>
                    ...
                    <TableRow key={event.id} className="hover:bg-slate-50/50 transition-colors">
                      <TableCell>
                        <Badge variant="outline" className={`capitalize font-bold ${
                          event.provider === 'resend' ? 'bg-slate-900 text-white' : 
                          event.provider.startsWith('zalo') ? 'bg-blue-600 text-white' : ''
                        }`}>
                          {event.provider}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="font-mono text-xs text-slate-500 truncate max-w-[120px]" title={event.dedupe_key}>
                          {event.dedupe_key?.substring(0, 16)}...
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={`font-semibold capitalize ${
                          event.status === 'received' ? 'border-emerald-200 text-emerald-700 bg-emerald-50' :
                          event.status === 'failed' ? 'border-rose-200 text-rose-700 bg-rose-50' :
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <DialogDescription className="mt-1 flex items-center gap-4">
                        <span>Provider: <strong className="uppercase">{selectedEvent.provider}</strong></span>
                        <span>Status: <strong className="uppercase">{selectedEvent.status}</strong></span>
                        <span>Time: <strong>{format(new Date(selectedEvent.received_at), 'dd/MM/yyyy HH:mm:ss')}</strong></span>
                      </DialogDescription>
                    </div>
                  </div>
                </DialogHeader>
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                      ...
                      <div className="font-mono text-slate-800 break-all">{selectedEvent.dedupe_key}</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                      <div className="text-slate-500 text-xs mb-1 font-semibold">Related Message ID</div>
                      <div className="font-mono text-slate-800 break-all">{selectedEvent.related_message_id || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                      <div className="text-slate-500 text-xs mb-1 font-semibold">Provider Event ID</div>
                      <div className="font-mono text-slate-800 break-all">{selectedEvent.provider_event_id || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                      <div className="text-slate-500 text-xs mb-1 font-semibold">Signature Valid</div>
                      <div className={`font-bold flex items-center gap-1 ${selectedEvent.signature_valid ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {selectedEvent.signature_valid ? <CheckCircle2 className="w-4 h-4"/> : <AlertCircle className="w-4 h-4"/>}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button 
                        ...
                        onClick={() => copyToClipboard(JSON.stringify(selectedEvent.headers_redacted, null, 2))}
                      >
                        <Copy className="w-3 h-3 mr-1" /> Copy JSON
                      </Button>
                    </div>
