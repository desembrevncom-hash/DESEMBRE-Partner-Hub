...
const CHECKLIST_GROUPS_V1_0_0B: ChecklistGroup[] = [
  {
    id: 'admin_route_smoke', name: '1. Admin Route Smoke', items: [
  ...
  {
    id: 'sale_tele_perm', name: '2. Sale/Tele Permission Smoke', items: [
      { id: 'perm_admin', name: 'Sale/Tele không vào /admin', expected: 'Chặn truy cập 403 / Redirect' },
      { id: 'perm_sender', name: 'Sale/Tele không vào Sender Accounts', expected: 'Không truy cập được config' },
      { id: 'perm_cust', name: 'Sale/Tele chỉ thấy customers thuộc quyền', expected: 'RLS hoạt động, không thấy khách của người khác' },
      { id: 'perm_mkt', name: 'Sale/Tele không chạy marketing', expected: 'Không có quyền send campaign' },
    ]
  },
  {
    id: 'provider_safety', name: '3. Provider Safety', items: [
      { id: 'ps_audience', name: 'Marketing Send to Audience locked', expected: 'Nút Send bị xám/disabled' },
      { id: 'ps_zns', name: 'Không gọi ZNS', expected: 'Luồng Zalo trả về lỗi hoặc chạy Sandbox' },
      { id: 'ps_email', name: 'Không gửi production email', expected: 'Send test chỉ nhận email whitelist' },
      { id: 'ps_gcal', name: 'Calendar không gửi GCal invite nếu không tick', expected: 'Không gọi Google API' },
    ]
  },
  {
    id: 'external_watch', name: '4. External Provider Watch', items: [
      { id: 'ew_gcal', name: 'Không có GCal invite tự động', expected: 'Kiểm tra network tab: Clean' },
      { id: 'ew_resend', name: 'Không gọi Resend production', expected: 'Network tab không có request' },
      { id: 'ew_zns', name: 'Không gọi ZNS', expected: 'Network tab không có request ZNS' },
      { id: 'ew_ai', name: 'Không gọi AI khi chưa bấm Generate', expected: 'Network sạch khi load form' },
      { id: 'ew_secret', name: 'Không có token/secret trong Network', expected: 'Responses không chứa credential thô' },
    ]
  },
  {
    id: 'admin_hub_safety', name: '5. Admin Hub Safety', items: [
      { id: 'ahs_ai', name: 'AI toggle không tự gọi provider', expected: 'Bật/tắt chỉ lưu DB' },
      { id: 'ahs_auto', name: 'Automation toggle an toàn', expected: 'Không tự bật/trigger job ngoài ý muốn' },
      { id: 'ahs_flags', name: 'Feature flag lưu chính xác', expected: 'Lưu đúng giá trị, không đổi cờ khác' },
      { id: 'ahs_warn', name: 'Có warning trước toggle nguy hiểm', expected: 'Hiển thị modal/confirm dialog' },
    ]
  },
  {
    id: 'data_integrity', name: '6. Data Integrity', items: [
  ...
  {
    id: 'perf', name: '7. Performance', items: [
      ...
      { id: 'pf_401', name: 'Không có 401/403 bất thường', expected: 'Network không bị chặn vô cớ' },
      { id: 'pf_req', name: 'Network provider request hợp lý', expected: 'Chỉ call khi user action' },
    ]
  }
];
...
ALL_LEGACY_ITEMS.forEach(item => {
  DEFAULT_LEGACY_DATA.items[item.id] = { status: 'NOT TESTED', tester: '', note: '', timestamp: '' };
});
...
ALL_V1_0_0B_ITEMS.forEach(item => {
  DEFAULT_V1_DATA.items[item.id] = { status: 'NOT TESTED', tester: '', note: '', evidence: '', timestamp: '' };
});
  ...
  useEffect(() => {
    ...
    if (storedLegacy) {
      try {
        ...
        const merged = { ...DEFAULT_LEGACY_DATA, ...parsed, items: { ...DEFAULT_LEGACY_DATA.items, ...(parsed.items || {}) } };
    ...
    if (storedV1) {
      try {
        ...
        const merged = { ...DEFAULT_V1_DATA, ...parsed, items: { ...DEFAULT_V1_DATA.items, ...(parsed.items || {}) } };
        setV1Data(merged);
      } catch (e) {
    ...
    const handleItemChange = (id: string, field: keyof ItemData, value: string) => {
      setData(prev => ({
        ...
        items: {
          ...prev.items,
          [id]: {
            ...(prev.items[id] || { status: 'NOT TESTED', tester: '', note: '', evidence: '', timestamp: '' }),
            [field]: value,
      ...
      <div className="animate-in fade-in duration-500 mt-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isV1 && (
              <Button variant="outline" onClick={handleCopyReport} className="text-indigo-600 border-indigo-200 hover:bg-indigo-50 rounded-xl">
                ...
                {!isCollapsed && (
                  <div className="divide-y divide-slate-100">
                    <div className="grid grid-cols-12 gap-3 px-4 py-2 bg-slate-50/40 text-[10px] font-black uppercase tracking-wider text-slate-400">
                      ...
                      {isV1 ? <div className="col-span-2">Evidence</div> : <div className="col-span-2">Updated</div>}
                      <div className="col-span-1 text-right">{isV1 ? 'Updated' : ''}</div>
                    </div>
                    
                    {group.items.map(item => {
                      const d = data.items[item.id] || { status: 'NOT TESTED', tester: '', note: '', evidence: '', timestamp: '' };
                      ...
                      return (
                        <div key={item.id} className={`grid grid-cols-12 gap-3 px-4 py-3 items-start hover:bg-slate-50/50 transition-colors ${isFailed ? 'bg-rose-50/30' : ''}`}>
                          ...
                          <div className="col-span-1 text-right text-[10px] font-medium text-slate-400 flex flex-col items-end">
                            {isV1 && d.timestamp ? new Date(d.timestamp).toLocaleDateString('vi-VN') : (isV1 ? '—' : '')}
                          </div>
                        </div>
                      );
                    })}
    ...
    <div className="max-w-7xl mx-auto p-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="mb-4">
          <TabsTrigger value="v1.0.0B" className="font-bold text-sm">v1.0 Smoke Test</TabsTrigger>
          <TabsTrigger value="legacy" className="font-bold text-sm">Legacy UAT</TabsTrigger>
        </TabsList>
        <TabsContent value="v1.0.0B">
          {renderChecklist(CHECKLIST_GROUPS_V1_0_0B, v1Data, setV1Data, setHasV1Changes, 'desembre:uat:v1.0.0B:smoke-test', hasV1Changes, true)}
        </TabsContent>
        <TabsContent value="legacy">
          {renderChecklist(LEGACY_CHECKLIST_GROUPS, legacyData, setLegacyData, setHasLegacyChanges, 'uatChecklist_v2', hasLegacyChanges, false)}
        </TabsContent>
      </Tabs>
    </div>
  );
