    ...
    <EditUnlockProvider>
      <div className="min-h-screen bg-slate-950 text-slate-100 pb-20 font-sans selection:bg-indigo-500 selection:text-white">
        ...
        <header className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-40">
          <div className="container mx-auto px-6 py-4 h-auto md:h-24 flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-7xl">
            <div className="flex items-center gap-6 w-full md:w-auto">
              <Link
                to="/"
                className="w-12 h-12 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-all shadow-2xl shrink-0"
              >
                <ArrowLeft className="w-6 h-6" />
              </Link>
              <div>
                <div className="flex items-center gap-2">
                  <Badge className="bg-indigo-500 text-white border-none text-[9px] font-black tracking-widest px-2 py-0.5 rounded-md">
                    ADMIN ONLY
                  </Badge>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
              ...
              >
                {({ loading }) => (
                  <Button
                    variant="outline"
                    className="h-12 px-6 rounded-2xl bg-slate-900 border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800 font-bold text-[10px] uppercase tracking-widest shrink-0"
                  >
                    {loading ? (
              ...
              {isManager && (
                <Button className="h-12 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-500/20 transition-all hover:scale-105 shrink-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center bg-slate-900/40 p-3 rounded-[32px] border border-slate-800/60 backdrop-blur-sm">
            ...
            <div className="lg:col-span-12 flex items-center justify-end px-4 py-2 border-t border-slate-800/50 mt-2 gap-4">
              <div className="flex items-center gap-3 bg-slate-950/50 px-4 py-2 rounded-2xl border border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[9px] font-bold uppercase transition-colors ${!vatOn ? "text-indigo-400" : "text-slate-600"}`}
                  >
                    Chưa VAT
                  </span>
                  <button
                  ...
                  >
                    <div
                      className={`w-3 h-3 rounded-full bg-white shadow-sm transition-all duration-300 ${vatOn ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                  <span
                    className={`text-[9px] font-bold uppercase transition-colors ${vatOn ? "text-indigo-400" : "text-slate-600"}`}
          ...
          <div className="bg-slate-900/20 rounded-[40px] border border-slate-800/50 overflow-hidden backdrop-blur-md shadow-2xl">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-slate-800/50 bg-slate-900/40">
                    <th className="px-6 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] w-16">
                      No.
                    </th>
                    <th className="px-6 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] w-28">
                      Visual
                    </th>
                    <th className="px-6 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
                      Sản phẩm & Mô tả
                    </th>
                    <th className="px-6 py-6 text-center text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] w-32">
                      Size
                    </th>
                    <th className="px-6 py-6 text-right text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] w-44">
                      Retail Consumer
                    </th>
                    <th className="px-6 py-6 text-right text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] w-44">
                      Salon Consumer
                          ...
                          <td className="px-6 py-6 max-w-md">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <Badge
                                  variant="outline"
                                  className="text-[9px] font-black text-slate-500 border-slate-800 py-0 uppercase"
                                >
                            ...
                            {retail ? (
                              <div className="space-y-1">
                                ...
                                <p className="text-[9px] font-bold text-slate-600 uppercase">
                                  NIÊM YẾT LẺ {vatOn ? "(ĐÃ CÓ VAT)" : ""}
                                </p>
                                <button
                                  onClick={() => handlePick(p.id, "retail")}
                                  className="text-[10px] px-3 py-1 rounded-md bg-indigo-500/20 text-indigo-400 font-bold uppercase hover:bg-indigo-500 hover:text-white transition-all"
                                >
                                  CHỌN LÊN ĐƠN
                                </button>
                              </div>
                          ...
                          <td className="px-6 py-6 text-right">
                            {salon ? (
                              <div className="space-y-1">
                                ...
                                <p className="text-[9px] font-bold text-slate-600 uppercase">
                                  GIÁ CHUYÊN NGHIỆP {vatOn ? "(ĐÃ CÓ VAT)" : ""}
                                </p>
                                <button
            ...
            <div className="px-8 py-6 border-t border-slate-800/50 bg-slate-900/40 flex items-center justify-between">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                Hiển thị <span className="text-white">{filteredProducts.length}</span> /{" "}
                {mergedProducts.length} sản phẩm
              </p>
              <div className="flex items-center gap-2">
                <Button variant="ghost" disabled className="text-[10px] font-black text-slate-600">
                  PREV
                </Button>
                <div className="flex items-center gap-1">
                  ...
                  <button className="w-8 h-8 rounded-lg bg-slate-800 text-slate-400 text-xs font-black hover:bg-slate-700 hover:text-white transition-all">
                    2
                  </button>
                </div>
                <Button
        ...
        {cart.length > 0 && (
          <div className="fixed bottom-8 right-8 z-50 animate-fade-in">
            <Button
              ...
              className="h-16 px-8 rounded-full shadow-[0_10px_40px_-10px_rgba(99,102,241,0.6)] bg-indigo-600 hover:bg-indigo-500 text-white font-black hover:scale-105 transition-all group"
            >
