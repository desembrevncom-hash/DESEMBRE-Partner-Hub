      ...
      <header className="bg-white/80 border-b border-slate-200/60 sticky top-0 z-20 backdrop-blur-md print:hidden">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between max-w-7xl">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              className="rounded-xl border-slate-200 font-bold text-xs h-10 px-6 shadow-sm hover:bg-slate-50"
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative">
              ...
              {getStatusSteps().map((step, idx) => {
                ...
                return (
                  <div
                    key={step.id}
                    className="relative z-1 flex flex-col items-center gap-3 group"
                  >
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center border-4 transition-all duration-500 ${
                        isActive
                          ? "bg-primary text-white border-primary/20 scale-110 shadow-xl shadow-primary/20"
                          : isCompleted
                    ...
                    <div className="text-center">
                      <p
                        className={`text-[11px] font-black uppercase tracking-widest ${isCompleted ? "text-slate-900" : "text-slate-300"}`}
                      >
                        {step.label}
                      </p>
                      {isActive && (
                      ...
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-all">
                        <td className="px-8 py-6">
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">
                            {item.size_type || "Retail"} {item.size}
                          </p>
                        </td>
              ...
              <CardContent className="p-8 pt-0 space-y-6">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-white text-indigo-500 text-[9px] font-black"
                    >
                      EXPRESS
                    </Badge>
                  </div>
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px] font-black"
                    >
                      ĐÃ THANH TOÁN
                    </Badge>
                  </div>
                ...
                <div className="pt-2 text-center">
                  <p className="text-[10px] text-slate-400 font-medium italic">
                    * Khi đơn hàng hoàn thành, hệ thống sẽ tự động lên lịch chăm sóc sau mua hàng
                    cho khách.
                  </p>
                </div>
              </CardContent>
...
function UpsellPanel({
  ...
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("product_cycle_settings") || "{}");
    } catch {
      return {};
    }
  });
  const [tierSettings, setTierSettings] = useState<any>(() => {
    try {
      return JSON.parse(localStorage.getItem("system_tier_settings") || "{}");
    } catch {
      return {};
    }
  });
    ...
    items.forEach((item) => {
      ...
      const matched = PRODUCTS.find(
        (p) =>
          ...
          (item.product_name &&
            p.name &&
            item.product_name.toLowerCase().includes(p.name.toLowerCase().slice(0, 15))) ||
          (item.product_name &&
            p.name &&
            p.name.toLowerCase().includes(item.product_name.toLowerCase().slice(0, 15))),
      );
      ...
      if (matched) {
        ...
        result.push({
          ...
          variant: variantMatch
            ? { size: variantMatch.size, price: variantMatch.price }
            : { size: item.size || "", price: item.unit_price || 0 },
        });
      } else {
  ...
  return (
    <Card
      className="rounded-[32px] border-none overflow-hidden"
        ...
        <div className="px-6 py-4 space-y-3">
          {suggestions.map((s, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-4 rounded-2xl bg-white/[0.08] hover:bg-white/[0.12] border border-white/10 transition-all group"
              >
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-emerald-400">
                          ≈{s.cycleDays} ngày/chu kỳ
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Clock className="w-3 h-3 text-amber-400" />
                        <span className="text-[10px] font-bold text-amber-400">
                          Nhắc sau {s.alertDays} ngày
                        </span>
                      </div>
                    </div>
        ...
        <div className="px-6 pb-4 space-y-3">
          {customerPhone && (
            ...
            >
              ...
              {customerName?.split(" ").slice(-1)[0] || "khách"}
            </a>
          )}
          <Link
            to="/orders/new"
        ...
        <div className="px-6 pb-6">
          <div className="p-3 rounded-xl bg-amber-400/10 border border-amber-400/20 flex items-start gap-2">
            ...
            <p className="text-[10px] text-amber-200 font-medium leading-relaxed">
              Gọi điện nhắc tái mua gối đầu trước khi khách cạn kiệt sản phẩm để tăng tỷ lệ chốt đơn
              lên 2-3x.
            </p>
          </div>
        </div>
