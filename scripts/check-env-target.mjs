...
async function run() {
  ...
  if (fs.existsSync(projectRefPath)) {
    ...
    log(
      "No supabase linked project ref found (.temp/project-ref missing). Skipping strict ref check.",
    );
  }
  ...
  try {
    ...
    if (trackedFiles.length > 0) {
      logError(
        `The following sensitive env file(s) are tracked by Git: ${trackedFiles.join(", ")}`,
      );
