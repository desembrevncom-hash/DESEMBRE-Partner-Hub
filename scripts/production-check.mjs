...
function log(msg)  { console.log(`[prod:readiness] ${msg}`); }
function ok(msg)   { console.log(`[prod:readiness] ✅ ${msg}`); }
function warn(msg) { console.warn(`[prod:readiness] ⚠️  ${msg}`); }
function fail(msg) { console.error(`[prod:readiness] ❌ FAIL: ${msg}`); }
...
if (fs.existsSync(migrationsDir)) {
  ...
  if (files.length === 0) {
    ...
    warn(`${files.length} migration file(s) found. Verify which have been applied to Production manually.`);
...
console.log(`  Branch valid              : ${isValidBranch ? '✅ Yes' : '⚠️  Warning (not release/master)'}`);
console.log(`  Supabase URL (prod ref)   : ${supabaseUrl.includes(PROD_PROJECT_REF) ? '✅ Confirmed' : supabaseUrl ? '❌ Missing' : '⚠️  Not set'}`);
console.log(`  Supabase URL (no staging) : ${supabaseUrl.includes(STAGING_PROJECT_REF) ? '❌ Still staging' : '✅ Clean'}`);
console.log(`  Migration files           : ${pendingMigrations.length} file(s) — verify applied status manually`);
console.log('  ────────────────────────────────────────────────────────────');
console.log('');
