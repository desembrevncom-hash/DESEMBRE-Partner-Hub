...
async function run() {
  ...
  console.log(
    ...
    settings?.chat_model,
    "RAG Filter:",
    settings?.rag_use_rpc_brand_filter,
  );

  console.log("=== C. Seed product_knowledge mẫu ===");
  // Find products in product_knowledge
  const { data: pk1 } = await supabase
    .from("product_knowledge")
    .select("id, product_id")
    .eq("product_id", 1)
    .single();
  const { data: pk2 } = await supabase
    .from("product_knowledge")
    .select("id, product_id")
    .eq("product_id", 2)
    .single();
  ...
  const { data: p1 } = await supabase
    ...
    .ilike("name", "%MILK ESSENTIAL CLEANSER%")
    .single();
  const { data: p2 } = await supabase
    .from("catalog_products")
    .select("id, brand_id, category_id")
    .ilike("name", "%WATER CLEANSER%")
    .single();
