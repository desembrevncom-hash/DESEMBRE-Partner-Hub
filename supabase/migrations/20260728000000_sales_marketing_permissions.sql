...
DROP POLICY IF EXISTS "Anyone view marketing campaigns" ON public.marketing_campaigns;
DROP POLICY IF EXISTS "Users view own or all marketing campaigns" ON public.marketing_campaigns;

-- Cho phép xem chiến dịch của chính mình hoặc tất cả nếu là admin
CREATE POLICY "Users view own or all marketing campaigns" 
...
DROP POLICY IF EXISTS "Users manage marketing campaigns" ON public.marketing_campaigns;
...
DROP POLICY IF EXISTS "Anyone view snapshots" ON public.campaign_recipient_snapshots;
DROP POLICY IF EXISTS "Users manage snapshots of their own campaigns" ON public.campaign_recipient_snapshots;

-- Cho phép Admin hoặc chính người tạo chiến dịch quản lý snapshots (bao gồm cả xem)
