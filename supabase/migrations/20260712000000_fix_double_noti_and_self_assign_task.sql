-- Fix double notifications (rely on trigger) and fix self-assigned tasks
...
BEGIN
    ...
    LOOP
        ...
        -- (Sẽ kích hoạt trigger tr_customer_ownership_assigned để tạo thông báo)
        ...
        -- 3. Tạo Task gọi điện (Bỏ qua điều kiện != v_admin_id để cho phép tự gán task cho bản thân)
        IF p_update_sale AND p_sale_id IS NOT NULL THEN
            INSERT INTO public.customer_tasks (
                ...
                'call',
                'high',
                -- Fix timezone issue by making it relative to now or specific time
