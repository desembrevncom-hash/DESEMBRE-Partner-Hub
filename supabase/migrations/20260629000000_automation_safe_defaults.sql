...
BEGIN
    ...
    BEGIN
        ...
        -- Log run
        INSERT INTO public.automation_run_logs (rule_id, status, matched_records, actions_taken)
...
BEGIN
    ...
    -- Thống kê trong ngày
    SELECT count(*), COALESCE(sum(actions_taken),0), count(*) FILTER (WHERE status='failed')
    ...
    FROM (
        SELECT l.id, l.rule_id, l.status, l.matched_records, l.actions_taken, l.error_message, l.created_at, r.name as rule_name
