-- =================================================================================
-- Fix Facebook Identity Functions Lint Errors
-- =================================================================================

-- 1. FIX: resolve_facebook_identity_manual_review
-- Fixes enum/text mismatch by using explicit CASE mapping.
-- Resolves 'ignored' status to 'unresolved' in resolver_status since 'ignored' doesn't exist there.
-- Changes to SECURITY DEFINER with search_path safety.
-- Drops old 4-parameter overload that was causing lingering lint errors.
...
BEGIN
    ...
    IF p_status = 'resolved' THEN
        ...
        IF v_social_profile_id IS NOT NULL THEN
            ...
            SET facebook_uid = p_numeric_uid,
                resolver_status = 'resolved'::public.resolver_status,
    ...
    IF p_status IN ('failed', 'ignored') THEN
        IF v_social_profile_id IS NOT NULL THEN
            UPDATE public.customer_social_profiles
            SET resolver_status = CASE p_status
                    WHEN 'failed' THEN 'failed'::public.resolver_status
                    WHEN 'ignored' THEN 'unresolved'::public.resolver_status
                    ELSE 'unresolved'::public.resolver_status
                END,
                resolver_method = 'manual_admin',
                updated_at = now()
            WHERE id = v_social_profile_id;
...
REVOKE ALL ON FUNCTION public.resolve_facebook_identity_manual_review(uuid, text, public.fb_resolution_job_status, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.resolve_facebook_identity_manual_review(uuid, text, public.fb_resolution_job_status, text, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.resolve_facebook_identity_manual_review(uuid, text, public.fb_resolution_job_status, text, text) TO authenticated;


-- 2. FIX: apply_facebook_name_to_customer
-- Fixes type cast warning by explicitly initializing empty text[] array instead of string '{}'.
-- Changes v_role to public.app_role to avoid casting and warnings.
-- Ensures SECURITY DEFINER rules are applied.
...
BEGIN
    ...
    IF v_role IN ('admin'::public.app_role, 'sub_admin'::public.app_role) THEN
        v_is_admin := true;
    END IF;
...
REVOKE ALL ON FUNCTION public.apply_facebook_name_to_customer(uuid, uuid, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.apply_facebook_name_to_customer(uuid, uuid, boolean) FROM anon;
