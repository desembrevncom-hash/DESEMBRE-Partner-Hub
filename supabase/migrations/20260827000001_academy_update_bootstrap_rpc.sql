-- Migration: Update Academy current student bootstrap RPC to include status
...
BEGIN
  ...
  IF v_student.customer_id IS NULL THEN
    RETURN jsonb_build_object(
      'state', 'NO_CUSTOMER',
      'student_account', jsonb_build_object('id', v_student.id, 'user_id', v_student.user_id, 'customer_id', v_student.customer_id, 'status', v_student.status),
      'customer', null,
      'active_membership', null,
      'tier', null,
      'latest_expired_membership', null
