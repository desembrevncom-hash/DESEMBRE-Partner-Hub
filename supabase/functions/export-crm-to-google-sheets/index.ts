...
Deno.serve(async (req) => {
  ...
  try {
    if (req.method !== "POST") {
      return json({ success: false, error: "Method not allowed", step: "init", details: "Only POST allowed" }, 405);
    }

    const authHeader = req.headers.get('Authorization');
    if (!authHeader) {
      return json({ success: false, error: "Missing Authorization header", step: "auth", details: "No token provided" }, 401);
    }
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return json({ success: false, error: "Missing Supabase server configuration", step: "env", details: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing" }, 500);
    }
    ...
    if (userError || !user) {
      return json({ success: false, error: "Unauthorized", step: "auth", details: userError?.message || "Invalid user token" }, 401);
    }
    ...
    if (rolesError) {
      return json({ success: false, error: "Failed to check permissions", step: "role", details: rolesError.message }, 500);
    }
    ...
    if (!isAdminOrSubAdmin) {
      return json({ success: false, error: "Forbidden: Admins only", step: "role", details: "User is not admin or sub_admin" }, 403);
    }
    ...
    if (logError) {
      ...
      return json({ success: false, error: "Failed to initialize sync log", step: "log", details: logError.message }, 500);
    }
    ...
    try {
      ...
      let privateKey = Deno.env.get("GOOGLE_PRIVATE_KEY") || Deno.env.get("GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY") || "";
      ...
      if (missing.length > 0) {
        throw { step: "env", error: "Missing Google Sheets configuration", details: `Missing: ${missing.join(', ')}` };
      }
      ...
      try {
        ...
        throw { step: "google_auth", error: "Failed to authenticate with Google API", details: e.message || "Invalid Private Key or Service Account" };
      }
      
      const accessToken = tokenResponse.token;

      if (!accessToken) {
        throw { step: "google_auth", error: "Failed to negotiate access token", details: "Token is empty" };
      }
      ...
      if (customersError) throw { step: "db_query", error: "Failed to fetch customers", details: customersError.message };

      const customersValues = [
        ["customer_id", "name", "business_name", "contact_name", "phone", "email", "city", "source_or_channel", "status", "lifecycle_stage", "owner_sale_id", "owner_tele_id", "last_contacted_at", "last_activity_at", "created_at", "data_health"]
      ];
      ...
      (customers || []).forEach(c => {
        ...
        customersValues.push([
          c.id, c.name, c.business_name || "", c.contact_name || "", c.phone || "", c.email || "",
          c.city || "", c.source || "", c.status || "", c.lifecycle_stage || "",
          c.owner_sale_id || "", c.owner_tele_id || "", c.last_contacted_at || "",
          c.last_activity_at || "", c.created_at || "", health
        ]);
      });
      ...
      if (logsError) throw { step: "db_query", error: "Failed to fetch import logs", details: logsError.message };

      const importLogsValues = [
        ["batch_id", "file_name", "status", "total_rows", "valid_rows", "invalid_rows", "duplicate_rows", "inserted_rows", "skipped_rows", "failed_rows", "created_at", "completed_at", "error_message"]
      ];

      (importLogs || []).forEach(log => {
        importLogsValues.push([
          log.id, log.file_name, log.status, String(log.total_rows || 0), String(log.valid_rows || 0),
          String(log.invalid_rows || 0), String(log.duplicate_rows || 0), String(log.inserted_rows || 0),
          String(log.skipped_rows || 0), String(log.failed_rows || 0), log.created_at || "",
          log.completed_at || "", log.error_message || ""
        ]);
      });
      ...
      if (unassignedError) throw { step: "db_query", error: "Failed to fetch unassigned customers", details: unassignedError.message };

      const unassignedValues = [
        ["customer_id", "name", "business_name", "contact_name", "phone", "email", "city", "source_or_channel", "status", "lifecycle_stage", "created_at", "last_contacted_at", "data_health"]
      ];
      ...
      (unassignedCustomers || []).forEach(c => {
         ...
         unassignedValues.push([
           c.id, c.name, c.business_name || "", c.contact_name || "", c.phone || "", c.email || "",
           c.city || "", c.source || "", c.status || "", c.lifecycle_stage || "",
           c.created_at || "", c.last_contacted_at || "", health
         ]);
      });
      ...
      // 5. Duplicate_Check
      // =====================================
      const { data: dupPhones, error: dupPhonesError } = await supabase.from('v_customers_duplicate_phone').select('*').limit(1000);
      const { data: dupEmails, error: dupEmailsError } = await supabase.from('v_customers_duplicate_email').select('*').limit(1000);

      if (dupPhonesError) throw { step: "db_query", error: "Failed to fetch duplicate phones", details: dupPhonesError.message };
      if (dupEmailsError) throw { step: "db_query", error: "Failed to fetch duplicate emails", details: dupEmailsError.message };
      ...
      (dupPhones || []).forEach(d => (d.customer_ids || []).forEach((id: string) => allDupIds.add(id)));
      (dupEmails || []).forEach(d => (d.customer_ids || []).forEach((id: string) => allDupIds.add(id)));
      ...
      for (let i = 0; i < dupIdsArray.length; i += 500) {
         ...
         const { data: chunkDetails, error: chunkError } = await supabase.from('customers').select('id, name, phone, email').in('id', chunk);
         if (chunkError) throw { step: "db_query", error: "Failed to fetch customer duplicates details", details: chunkError.message };
      ...
      const processDuplicates = (type: string, list: any[]) => {
         (list || []).forEach(d => {
            ...
            let displayNames = details.map((x: any) => x.name).slice(0, 10).join(', ');
            let displayPhones = details.map((x: any) => x.phone || '').slice(0, 10).join(', ');
            let displayEmails = details.map((x: any) => x.email || '').slice(0, 10).join(', ');

            if (ids.length > 10) {
               displayIds += ', ...'; displayNames += ', ...'; displayPhones += ', ...'; displayEmails += ', ...';
            }
            
            duplicateValues.push([
               type, type === 'Phone' ? d.normalized_phone : d.normalized_email, String(d.duplicate_count),
               displayIds, displayNames, displayPhones, displayEmails
            ]);
         });
      };
      ...
      const [
        ...
        supabase.from('customers').select('*', { count: 'exact', head: true }).gte('created_at', todayStr),
        supabase.from('customers').select('*', { count: 'exact', head: true }).gte('created_at', d7.toISOString()),
        supabase.from('customers').select('*', { count: 'exact', head: true }).is('owner_sale_id', null).is('owner_tele_id', null),
        supabase.from('customer_import_batches').select('*', { count: 'exact', head: true }),
        supabase.from('customer_import_batches').select('*', { count: 'exact', head: true }).eq('status', 'completed'),
        supabase.from('customer_import_batches').select('*', { count: 'exact', head: true }).eq('status', 'failed'),
        supabase.from('customer_import_batches').select('created_at').order('created_at', { ascending: false }).limit(1).maybeSingle(),
        supabase.from('crm_sync_logs').select('completed_at').eq('status', 'success').order('completed_at', { ascending: false }).limit(1).maybeSingle()
      ]);

      if (err1 || err2 || err3 || err4 || err5 || err6 || err7 || err8 || err9) {
        throw { step: "db_query", error: "Failed to fetch daily summary metrics", details: "Multiple queries failed" };
      }
      ...
      const dailySummaryValues = [
         ...
         ["new_customers_7_days", String(newCustomers7DaysCount || 0), "Customers created in last 7 days"],
         ...
         ["completed_import_batches", String(completedBatchesCount || 0), "Batches successfully confirmed"],
         ...
         ["last_google_sheet_sync_at", lastSyncData?.completed_at || "N/A", "Last successful Google Sheet sync"]
      ];

      // =====================================
      // 7. Remarketing Email Ready & 8. Remarketing Zalo Ready
      // =====================================
      const { data: consents, error: consentsError } = await supabase.from('customer_consents').select('*');
      const { data: zaloProfiles, error: zaloError } = await supabase.from('customer_zalo_profiles').select('*');

      if (consentsError) throw { step: "db_query", error: "Failed to fetch consents", details: consentsError.message };
      if (zaloError) throw { step: "db_query", error: "Failed to fetch zalo profiles", details: zaloError.message };
      ...
      const emailReadyValues = [
        ["customer_id", "name", "email", "phone", "source_or_channel", "status", "lifecycle_stage", "consent_status", "opt_in_at", "consent_source", "last_contacted_at", "reason"]
      ];
      
      const zaloReadyValues = [
        ["customer_id", "name", "phone", "email", "zalo_user_id", "oa_follow_status", "source_or_channel", "status", "lifecycle_stage", "consent_status", "opt_in_at", "consent_source", "last_contacted_at", "reason"]
      ];
      ...
      (customers || []).forEach(c => {
        ...
        // Email Ready Check
        if (!isBlocked && !isDuplicate && !hasOptOut && emailConsent && emailConsent.is_opt_in === true && c.email && c.email.includes('@')) {
           if (emailReadyValues.length <= 5000) {
             emailReadyValues.push([
               c.id, c.name, c.email || "", c.phone || "", c.source || "", c.status || "", c.lifecycle_stage || "",
               "opt_in", emailConsent.opt_in_at || "", emailConsent.source || "", c.last_contacted_at || "", "Đủ điều kiện"
             ]);
           }
        }
        ...
        const hasZaloReady = (zaloConsent && zaloConsent.is_opt_in === true) || (profile && profile.zalo_id);
        if (!isBlocked && !isDuplicate && !hasOptOut && hasZaloReady && c.phone && c.phone.length >= 9) {
           if (zaloReadyValues.length <= 5000) {
             zaloReadyValues.push([
               c.id, c.name, c.phone || "", c.email || "", profile?.zalo_id || "", profile?.oa_follow_status || "", c.source || "", c.status || "", c.lifecycle_stage || "",
               "opt_in", zaloConsent?.opt_in_at || "", zaloConsent?.source || "", c.last_contacted_at || "", "Đủ điều kiện"
             ]);
           }
        }
      });
      ...
      const requiredTabs = ["Customers_Master", "Data_Quality", "Import_Logs", "Unassigned_Customers", "Duplicate_Check", "Daily_Summary", "Remarketing_Email_Ready", "Remarketing_Zalo_Ready"];
      ...
      if (getSheetRes.ok) {
        ...
        if (tabsToCreate.length > 0) {
          ...
          if (!createTabsRes.ok) {
            ...
            throw { step: "tabs", error: "Failed to auto-create missing tabs", details: errData.error?.message || "Unknown error" };
          }
        }
      } else {
        const errData = await getSheetRes.json();
        throw { step: "tabs", error: "Failed to fetch spreadsheet info", details: errData.error?.message || "Spreadsheet might not exist or no permission" };
      }
      ...
      try {
        sheetResponse = await fetch(updateUrl, {
         ...
         throw { step: "write", error: "Network error writing to Google Sheets", details: e.message };
      }
    ...
    return json({ success: false, error: "Internal Server Error", step: "fatal", details: error.message || "Unknown fatal error" }, 500);
  }
});
