...
serve(async (req) => {
  ...
  try {
    ...
    const { data: hasPilotAccess } = await supabaseUser.rpc("check_pilot_access", {
      p_module_key: "ai_customer_suggestions",
      p_user_id: user.id,
    });
    if (!hasPilotAccess) {
      return new Response(
    ...
    const { data: hasAccess } = await supabaseUser.rpc("can_view_customer", {
      p_customer_id: customerId,
      p_user_id: user.id,
    });
    const { data: isAdmin } = await supabaseUser.rpc("is_admin_or_sub_admin", {
      p_user_id: user.id,
    });

    if (!hasAccess && !isAdmin) {
      return new Response(JSON.stringify({ error: "Access denied to this customer" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: customer } = await supabaseAdmin
      ...
      .eq("id", customerId)
      .single();
    const { data: timeline } = await supabaseAdmin.rpc("get_customer_timeline", {
      p_customer_id: customerId,
    });
    ...
    const customerContext = {
      ...
      channels: channels?.map((c: any) => ({
        type: c.channel_type,
        purpose: c.channel_purpose,
        status: c.resolve_status,
      })),
      recent_timeline: Array.isArray(timeline)
...
`;
    ...
    const { data: savedSuggestion, error: insertErr } = await supabaseAdmin
