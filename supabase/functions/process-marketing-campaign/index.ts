...
serve(async (req) => {
  ...
  try {
    ...
    const { data: campaign, error: campErr } = await adminClient
      ...
      .select("id, channel, approval_status, segment_id, draft_subject, draft_body, sender_account_id")
      .eq("id", campaign_id)
      .single();

    if (campErr || !campaign) {
      return new Response(JSON.stringify({ success: false, error: "Campaign not found", step: "campaign_check" }), {
        status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }

    if (campaign.approval_status !== "approved") {
      return new Response(JSON.stringify({ success: false, error: "Campaign phải được duyệt (approved) mới có thể gửi production.", step: "approval_check" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    ...
    if (campaign.segment_id) {
      const { data: mapData } = await adminClient
        .from("customer_segments_map").select("customer_id").eq("segment_id", campaign.segment_id);
      if (mapData && mapData.length > 0) {
        const cIds = mapData.map((m: any) => m.customer_id);
        const { data: cData } = await adminClient.from("customers").select("id, name, email, phone, marketing_opt_in, marketing_opt_out_at, is_active").in("id", cIds).limit(10000);
        if (cData) customers = cData;
      }
    } else {
      const { data: cData } = await adminClient.from("customers").select("id, name, email, phone, marketing_opt_in, marketing_opt_out_at, is_active").limit(10000);
      if (cData) customers = cData;
    }
    ...
    if (campaign.channel === "zalo" || campaign.channel === "zalo_oa" || campaign.channel === "zalo_zns") {
      // Chunking if too many
      for (let i = 0; i < customerIds.length; i += 1000) {
        const chunk = customerIds.slice(i, i + 1000);
        const { data: zData } = await adminClient.from("customer_zalo_profiles").select("customer_id, zalo_id, is_following_oa, zalo_phone").in("customer_id", chunk);
        if (zData) {
    ...
    const { eligible_count, excluded_counts, preview_recipients, eligible_recipients } = buildEligibleAudience(
      customers,
      campaign.channel,
      zaloProfilesMap,
      suppressionSet,
    ...
    // 8. Rate Limit Logic
    const max_batch_size = (campaign.channel === "email" || campaign.channel === "email_campaign") ? 100 : 10;
    ...
    const { data: latestCamp } = await adminClient.from("marketing_campaigns").select("final_confirmed_at, paused_at").eq("id", campaign_id).single();
    if (!latestCamp?.final_confirmed_at) {
      return new Response(JSON.stringify({ success: false, error: "Campaign chưa được xác nhận Final Confirmation.", step: "final_confirmation" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }

    if (latestCamp?.paused_at) {
      return new Response(JSON.stringify({ success: false, error: "Campaign đang bị tạm dừng.", step: "paused" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    ...
    if (providerMode === "resend_pilot") {
      ...
      if (campaign.channel !== "email" && campaign.channel !== "email_campaign") {
        return new Response(JSON.stringify({ success: false, error: "Pilot mode only supports Email channel.", step: "pilot_validation" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }});
      }
      ...
      if (whitelist.length === 0 || whitelistStr === "") {
        return new Response(JSON.stringify({ success: false, error: "Whitelist rỗng, không thể chạy Pilot.", step: "pilot_whitelist" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }});
      }

      eligible_recipients = eligible_recipients.filter(r => r.email && whitelist.includes(r.email.toLowerCase()));

      if (eligible_recipients.length === 0) {
        return new Response(JSON.stringify({ success: false, error: "Không tìm thấy khách hàng nào trong audience có email trùng với whitelist nội bộ. (no_internal_pilot_recipient)", step: "pilot_filter" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }});
      }

      if (eligible_recipients.length > 5) {
        return new Response(JSON.stringify({ success: false, error: "Số lượng pilot recipients vượt quá 5. (pilot_limit_exceeded)", step: "pilot_filter" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }});
      }
      ...
      for (const rec of eligible_recipients) {
        // Prepare template
        const finalSubject = renderTemplate(campaign.draft_subject, { customer_name: rec.name || "Customer" });
        const finalBody = renderTemplate(campaign.draft_body, { customer_name: rec.name || "Customer" });
        ...
        logInserts.push({
          ...
          delivery_metadata: {
            ...
            provider: "resend",
            safety_checks: { suppression_checked: true, duplicate_checked: true, consent_checked: true },
            consent_snapshot: { marketing_opt_in: rec.marketing_opt_in, opt_out_at: rec.marketing_opt_out_at },
    ...
    return new Response(JSON.stringify({
      ...
      eligible_count,
      skipped_count: excluded_counts.opt_out + excluded_counts.no_consent + excluded_counts.duplicate,
