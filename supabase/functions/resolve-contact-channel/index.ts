...
serve(async (req) => {
  ...
  try {
      ...
      resolveStatus = 'manual';
    } else if (channelType === 'tiktok' || channelType === 'instagram' || channelType === 'facebook') {
       ...
       try {
         ...
         if (channelType === 'facebook') {
            if (!url.hostname.includes('facebook.com') && !url.hostname.includes('fb.com') && !url.hostname.includes('fb.watch')) {
    ...
    if (channelType === 'facebook' && metaToken) {
       ...
       const match = normalizedValue.match(/(?:facebook\.com|fb\.com)\/(?:profile\.php\?id=)?([a-zA-Z0-9.\-]+)/);
       if (match && match[1]) {
           const idOrUsername = match[1];
           try {
             const fbRes = await fetch(`https://graph.facebook.com/v19.0/${idOrUsername}?access_token=${metaToken}`);
    ...
    // Log Activity
    const activityTitle = scope === 'official' ? 'Cập nhật kênh liên hệ chính thức' : 'Thêm kênh liên hệ riêng';
