...
serve(async (req) => {
  ...
  if (!isInternalCall && authHeader) {
    ...
    if (!isAdmin) return new Response(JSON.stringify({ success: false, error: "Chỉ Admin/SubAdmin mới có thể process retry" }), { status: 403, headers: corsHeaders });
  }
  ...
  if (fetchErr) return new Response(JSON.stringify({ success: false, error: fetchErr.message }), { headers: corsHeaders });

  if (!pendingRetries || pendingRetries.length === 0) {
    return new Response(JSON.stringify({ success: true, processed: 0, message: "Không có retry nào cần xử lý" }), { headers: corsHeaders });
  }
  ...
  for (const retry of pendingRetries) {
    // Mark as retrying
    await adminClient.from("marketing_retry_queue").update({ status: "retrying", updated_at: new Date().toISOString() }).eq("id", retry.id);

    const newCount = retry.retry_count + 1;

    try {
      // Load current sender token
      const { data: tokenRow } = await adminClient.from("sender_account_tokens")
        .select("*").eq("sender_account_id", retry.sender_account_id).maybeSingle();
      ...
      if (isExpired) {
        ...
        const { data: newRow } = await adminClient.from("sender_account_tokens")
          .select("access_token_enc").eq("sender_account_id", retry.sender_account_id).maybeSingle();
      ...
      const { data: template } = await adminClient.from("zns_templates")
        .select("zalo_template_id, template_name, required_params").eq("id", retry.zns_template_id).maybeSingle();
      if (!template) throw new Error("Template not found");

      // Load customer phone
      const { data: customer } = await adminClient.from("customers")
        .select("phone").eq("id", retry.customer_id).maybeSingle();
      ...
      const znsRes = await fetch("https://business.openapi.zalo.me/message/template", {
        ...
        body: JSON.stringify({ phone: formattedPhone, template_id: template.zalo_template_id, template_data: retry.payload }),
        signal: AbortSignal.timeout(10000),
      });

      const znsResult = await znsRes.json();

      if (znsResult.error !== 0) throw new Error(`Zalo error ${znsResult.error}: ${znsResult.message}`);

      // Success — update queue and log
      await adminClient.from("marketing_retry_queue").update({ status: "succeeded", retry_count: newCount, updated_at: new Date().toISOString() }).eq("id", retry.id);
      if (retry.delivery_log_id) {
        await adminClient.from("marketing_delivery_logs").update({ status: "sent", retry_count: newCount, last_retry_at: new Date().toISOString(), provider_message_id: znsResult.data?.message_id, provider_response: znsResult }).eq("id", retry.delivery_log_id);
      }
      await adminClient.from("sender_action_logs").insert({ action: "zns_retry_processed", sender_type: "business", result: "sent", note: `Retry #${newCount} succeeded` });
      results.push({ id: retry.id, status: "succeeded", attempt: newCount });

    } catch (err: any) {
      // Check if we've hit max retries
      if (newCount >= retry.max_retries) {
        await adminClient.from("marketing_retry_queue").update({ status: "abandoned", retry_count: newCount, updated_at: new Date().toISOString() }).eq("id", retry.id);
        if (retry.delivery_log_id) {
          await adminClient.from("marketing_delivery_logs").update({ status: "abandoned", retry_count: newCount, last_retry_at: new Date().toISOString() }).eq("id", retry.delivery_log_id);
        }
        await adminClient.from("sender_action_logs").insert({ action: "zns_retry_abandoned", sender_type: "business", result: "abandoned", note: `After ${newCount} attempts: ${err.message}` });
        results.push({ id: retry.id, status: "abandoned", attempt: newCount });
      } else {
        // Re-queue with backoff
        const backoffMinutes = [5, 15, 45, 120][newCount] ?? 120;
        const nextRetry = new Date(Date.now() + backoffMinutes * 60 * 1000).toISOString();
        await adminClient.from("marketing_retry_queue").update({
          status: "pending", retry_count: newCount, next_retry_at: nextRetry, updated_at: new Date().toISOString(),
        }).eq("id", retry.id);
        if (retry.delivery_log_id) {
          await adminClient.from("marketing_delivery_logs").update({ status: "retrying", retry_count: newCount, last_retry_at: new Date().toISOString() }).eq("id", retry.delivery_log_id);
        }
        results.push({ id: retry.id, status: "requeued", attempt: newCount, next_retry: nextRetry });
      }
    }
  }
