  ...
  try {
    ...
    if (xWorkerSecret) {
      ...
      if (!cronSecret || xWorkerSecret !== cronSecret) {
        return new Response(JSON.stringify({ error: "Unauthorized (invalid worker secret)" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      isCronAuth = true;
      
      // Kill switch check for cron
      if (!isWorkerEnabled) {
        return new Response(JSON.stringify({ success: true, skipped: true, reason: "worker_disabled" }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
    } else if (authHeader) {
    ...
    const { data: lockAcquired, error: lockError } = await supabaseAdmin
      .rpc('acquire_execution_lock', { p_lock_key: LOCK_KEY, p_ttl_seconds: LOCK_TTL_SECONDS });
    ...
    try {
      ...
      for (const event of (events || [])) {
        try {
          ...
          let email = payload?.data?.to?.[0] || payload?.data?.email_address || payload?.data?.email;
          ...
          if (event.event_type === 'email.delivered') {
            ...
            newDeliveryStatus = 'failed'; newDeliveryReason = 'bounced';
            isSuppressionEvent = true; suppressionReason = 'bounced';
          } else if (event.event_type === 'email.complained') {
            newDeliveryStatus = 'failed'; newDeliveryReason = 'complaint';
            isSuppressionEvent = true; suppressionReason = 'complaint';
            ...
            if (type === 'hard_bounce' || failureReason.includes('permanent') || failureReason.includes('rejected')) {
              newDeliveryStatus = 'failed'; newDeliveryReason = 'provider_failed_permanent';
              isSuppressionEvent = true; suppressionReason = 'provider_failed_permanent';
            } else {
              if (!isDryRun) {
                await supabaseAdmin.from('webhook_events').update({
                  status: 'ignored',
            ...
            if (dLog) {
              ...
              if (newPri > currentPri) {
                if (isDryRun) {
                  ...
                  await supabaseAdmin.from('marketing_delivery_logs').update(updateData).eq('id', dLog.id);
            ...
            if (isDryRun) {
              ...
              if (existingSuppression) {
                ...
                if (newPriority > currentPriority) {
                  await supabaseAdmin.from('marketing_suppression_list').update({
